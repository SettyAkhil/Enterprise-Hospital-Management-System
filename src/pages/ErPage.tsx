...
export default function ErPage({
  ...
  const filteredVisits = useMemo(() => {
    ...
    // 3. Patient Type Filter (New vs Existing)
    if (patientTypeFilter === "new") {
      list = list.filter((v) => !v.patient_id || (v as any).is_new_patient || v.is_unknown_patient);
    } else if (patientTypeFilter === "existing") {
      list = list.filter((v) => Boolean(v.patient_id) && !(v as any).is_new_patient && !v.is_unknown_patient);
    }
  ...
  }, [visits, trackboardSearch, acuityFilter, stageFilter, patientTypeFilter]);
    ...
    <div className="flex-1 bg-[#F8FAFC] p-4 sm:p-6 space-y-3.5 min-h-full font-sans text-slate-900">
      ...
      <div className="bg-slate-900 text-white rounded-xl p-3.5 sm:p-4 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-3 border border-slate-800">
        ...
        <div className="flex flex-wrap items-center gap-2.5 shrink-0 self-start md:self-auto">
          {/* Direct Quick Action: ＋ New Patient */}
          <button
            type="button"
            onClick={() => setIntakeModalType("new")}
            className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg shadow-2xs transition-all flex items-center gap-1.5 cursor-pointer"
          ...
          {/* Direct Quick Action: ↻ Existing Patient */}
          <button
            type="button"
            onClick={() => setIntakeModalType("existing")}
            className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-100 text-xs font-bold rounded-lg border border-slate-700 shadow-2xs transition-all flex items-center gap-1.5 cursor-pointer"
            title="Search and Admit Existing Patient (↻)"
          >
            <span className="text-[15px] font-black leading-none">↻</span>
          ...
          <button
            type="button"
            onClick={loadVisits}
            className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-lg border border-slate-700 shadow-2xs transition-all flex items-center gap-1.5 cursor-pointer"
            title="Refresh ER visits"
          >
            <FiRefreshCw className="text-[12px]" /> Refresh
          </button>

          {/* Full Intake Dropdown */}
          <div className="relative" ref={regMenuRef}>
            ...
            {isRegMenuOpen && (
              <div className="absolute right-0 mt-2 w-72 bg-white border border-[#DDE2EC] rounded-lg shadow-xl z-50 py-1.5 overflow-hidden text-slate-900">
                ...
                >
                  <div className="w-8 h-8 rounded bg-blue-100 text-[#1B4FD8] flex items-center justify-center text-[16px] font-black shrink-0 mt-0.5 group-hover:bg-[#1B4FD8] group-hover:text-white transition-colors">
                    ＋
                  </div>
                  <div>
                    <div className="text-[13px] font-bold text-gray-900 group-hover:text-[#1B4FD8] flex items-center gap-1">
                      <span className="font-black text-[#1B4FD8] text-sm">＋</span>
                      <span>New Patient</span>
                    </div>
                ...
                >
                  <div className="w-8 h-8 rounded bg-slate-100 text-slate-700 flex items-center justify-center text-[16px] font-black shrink-0 mt-0.5 group-hover:bg-[#1B4FD8] group-hover:text-white transition-colors">
                    ↻
                  </div>
                  <div>
                    <div className="text-[13px] font-bold text-gray-900 group-hover:text-[#1B4FD8] flex items-center gap-1">
        ...
        <div className="bg-red-600/10 border-l-4 border-l-red-600 border border-red-200 rounded-lg p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-red-900 shadow-2xs">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-semibold">
              ...
              patient{acuityCounts.B1 + acuityCounts.B2 > 1 ? "s" : ""} in department. Immediate clinical priority.
            </span>
          </div>
          <button
            type="button"
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden">
        ...
        <div className="p-3 sm:p-4 border-b border-slate-200 bg-slate-50/70 space-y-3">
          {/* Top Row: Search, Patient Type Filters & Scope */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {/* Patient Type Filter (＋ New Patient / ↻ Existing Patient) & Scope */}
            <div className="flex flex-wrap items-center gap-2 shrink-0">
              <div className="flex items-center gap-1 bg-white border border-slate-300 p-0.5 rounded-lg shadow-2xs text-xs">
                <button
                  type="button"
                  onClick={() => setPatientTypeFilter("all")}
                  className={`px-2 py-1 rounded-md text-[11px] font-bold transition-all cursor-pointer ${
                    patientTypeFilter === "all" ? "bg-slate-800 text-white shadow-2xs" : "text-slate-600 hover:bg-slate-50"
                  }`}
                >
                  All
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md text-[11px] font-bold transition-all cursor-pointer flex items-center gap-1 ${
                    patientTypeFilter === "new" ? "bg-blue-600 text-white shadow-2xs" : "text-blue-700 hover:bg-blue-50"
                  }`}
                  title="Filter New Patients (＋)"
                >
                ...
                <button
                  type="button"
                  onClick={() => setPatientTypeFilter("existing")}
                  className={`px-2.5 py-1 rounded-md text-[11px] font-bold transition-all cursor-pointer flex items-center gap-1 ${
                    patientTypeFilter === "existing" ? "bg-slate-700 text-white shadow-2xs" : "text-slate-700 hover:bg-slate-50"
                  }`}
                  title="Filter Existing Patients (↻)"
                >
                  <span className="font-black">↻</span>
                  <span>Existing Patient</span>
                </button>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Click &quot;＋ New Patient&quot; or &quot;↻ Existing Patient&quot; in the header above to admit an emergency patient.
            </p>
          </div>
          ...
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Try resetting the acuity, patient type, or stage filters, or clearing the search box.
            </p>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs">
                {filteredVisits.map((v) => {
                  ...
                  const isExistingPatient = Boolean(v.patient_id) && !anyV.is_new_patient && !v.is_unknown_patient;

                  return (
                    <tr
                      key={v.id}
                      onClick={() => {
                        // Open in Triage per user specification
                        if (onOpenTriage) {
                          onOpenTriage(v.id);
                        } else {
                    ...
                    >
                        ...
                        {v.is_unknown_patient ? (
                          ...
                          <div>
                            <div className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                              ...
                              {/* Patient Type Badge with Symbol: ＋ New or ↻ Existing */}
                              {isExistingPatient ? (
                                <span
                                  className="inline-flex items-center gap-0.5 px-1.5 py-0.2 text-[10px] font-bold text-slate-600 bg-slate-100 rounded border border-slate-200 shrink-0"
                                ...
                                <span
                                  className="inline-flex items-center gap-0.5 px-1.5 py-0.2 text-[10px] font-bold text-blue-700 bg-blue-50 rounded border border-blue-200 shrink-0"
                                  title="New Patient (＋)"
                                >
                                  <span>＋</span>
                                  <span>New</span>
                      ...
                      <td className="pr-4 py-3 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className="px-3 py-1.5 text-xs font-bold text-white bg-[#1B4FD8] hover:bg-blue-700 rounded-lg shadow-2xs hover:shadow-xs transition-all cursor-pointer inline-flex items-center gap-1.5"
                          title="Open Patient in Triage"
                        >
