...
const PAYMENT_MODES: NonNullable<LabOrder["billing"]["mode"]>[] = ["Cash", "Card", "UPI", "Insurance", "Corporate"];

export default function LabBillingQueue({ collectedBy = "Reception" }: { collectedBy?: string }) {
  const [tick, setTick] = useState(0);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [mode, setMode] = useState<NonNullable<LabOrder["billing"]["mode"]>>("Cash");
  ...
  const settled = useMemo(
    () => LabOrderDatabase.getOrders().filter(order => order.billing.status === "Paid"),
    [tick]
  );
  ...
  const collect = () => {
    ...
    const updated = LabOrderDatabase.markBilled(selected.id, { mode, discount, collectedBy });
    if (!updated) return;
  ...
  const pendingTotal = pending.reduce((sum, order) => sum + order.billing.subtotal, 0);

  return (
    <div className="flex-1 overflow-y-auto bg-[#F0F2F5]">
      <div className="bg-white border-b border-[#DDE2EC] px-6 py-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-base font-semibold text-gray-900">Lab Test Billing</h1>
          <p className="text-[11.5px] text-[#64748B]">
            Investigations ordered by doctors · payment releases them to the laboratory
          </p>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-right">
            <div className="text-[11px] text-[#94A3B8] uppercase tracking-wide font-bold">Awaiting payment</div>
      ...
      <div className="bg-white border-b border-[#DDE2EC] flex">
        {([
          { key: "pending" as const, label: "Awaiting Billing", count: pending.length },
          ...
          <button
            ...
            className={`px-5 py-2.5 text-[12.5px] font-semibold border-b-2 transition-colors ${
              view === tab.key ? "border-[#1B4FD8] text-[#1B4FD8]" : "border-transparent text-[#64748B] hover:text-[#334155]"
            }`}
          >
            {tab.label}
            <span className="ml-1.5 text-[10px] font-bold bg-[#F1F5F9] text-[#475569] px-1.5 py-0.5 rounded">{tab.count}</span>
          </button>
        ))}
      </div>
      ...
      {receipt && (
        <div className="mx-5 mt-4 bg-[#F0FDF4] border border-[#BBF7D0] rounded px-4 py-3 flex flex-wrap items-center justify-between gap-2">
          <div className="text-[12.5px] text-[#166534]">
            ...
            <span className="font-mono font-bold">{receipt.billing.receiptNo}</span>, ₹
            {receipt.billing.total.toLocaleString("en-IN")} for {receipt.patientName}. {receipt.id} is now on the
            laboratory worklist.
          </div>
          <button type="button" onClick={() => setReceipt(null)} className="text-[#15803D] font-bold text-[12px]">
            ✕
          </button>
        </div>
      )}
        ...
        <div className="lg:col-span-2 bg-white border border-[#DDE2EC] rounded">
          <div className="px-4 py-2.5 border-b border-[#DDE2EC]">
            <h2 className="text-[13px] font-bold text-gray-900">
              {view === "pending" ? "Orders waiting on payment" : "Settled orders"}
            </h2>
          </div>
          ...
          {list.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-[12.5px] font-semibold text-[#334155]">
                {view === "pending" ? "Nothing waiting on billing" : "No settled orders yet"}
              </p>
              <p className="text-[11.5px] text-[#94A3B8] mt-1">
                Lab tests appear here the moment a doctor dispatches a consultation sheet.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11.5px] text-[#64748B] mt-0.5">
                        {order.doctorName} · {order.department} · {order.diagnosis || "No diagnosis recorded"}
                      </div>
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <div className="text-[10.5px] text-[#94A3B8] font-mono">{order.id}</div>
                      <div className="mt-1">
        ...
        <div className="space-y-4">
          {selected ? (
            <div className="bg-white border border-[#DDE2EC] rounded">
              <div className="px-4 py-2.5 border-b border-[#DDE2EC]">
                <h2 className="text-[13px] font-bold text-gray-900">
                  {selected.billing.status === "Paid" ? "Receipt" : "Collect payment"}
                </h2>
              ...
              <div className="p-4 space-y-3">
                <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded p-3 space-y-1">
                  {[
                    ["Patient", `${selected.patientName} (${selected.age}${selected.sex?.[0] || ""})`],
                    ["UMR / OP", `${selected.umr} · ${selected.opNumber}`],
                    ["Phone", selected.phone || "Not recorded"],
                    ["Referred by", `${selected.doctorName}, ${selected.department}`],
                  ].map(([label, value]) => (
                ...
                <div className="space-y-1">
                  {selected.tests.map(test => (
                    <div key={test.id} className="flex justify-between gap-3 text-[12px] py-1 border-b border-[#F8FAFC] last:border-0">
                      <span className="text-[#475569]">
                        {test.name}
                        {test.urgency === "STAT" && <span className="text-[#DC2626] font-bold ml-1">STAT</span>}
                      </span>
                      <span className="font-mono text-gray-900">₹{test.price.toLocaleString("en-IN")}</span>
                    </div>
                  ))}
                </div>
                ...
                {selected.billing.status === "Paid" ? (
                  <div className="bg-[#F0FDF4] border border-[#BBF7D0] rounded p-3 text-[12px] text-[#166534] space-y-1">
                    <div className="flex justify-between">
                      <span>Paid</span>
                      <span className="font-mono font-bold">₹{selected.billing.total.toLocaleString("en-IN")}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Mode</span>
                    ...
                    <div className="flex justify-between">
                      <span>Receipt</span>
                      <span className="font-mono">{selected.billing.receiptNo}</span>
                    </div>
                    <div className="flex justify-between">
                  ...
                  <>
                    <div className="flex justify-between text-[12px] pt-1">
                      ...
                      <span className="font-mono text-gray-900">₹{selected.billing.subtotal.toLocaleString("en-IN")}</span>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={discount}
                        onChange={event =>
                          setDiscount(Math.max(0, Math.min(selected.billing.subtotal, parseInt(event.target.value, 10) || 0)))
                        }
                    ...
                    <div className="flex justify-between items-center border-t border-[#DDE2EC] pt-3">
                      <span className="text-[12px] font-semibold text-[#334155]">Amount due</span>
                      <span className="text-[18px] font-bold text-gray-900 font-mono">
                        ₹{(selected.billing.subtotal - discount).toLocaleString("en-IN")}
                      </span>
                    </div>

                    <Btn variant="primary" size="sm" onClick={collect} className="w-full">
            ...
            <div className="bg-white border border-[#DDE2EC] rounded p-6 text-center">
              <p className="text-[12.5px] text-[#94A3B8]">Select an order to take payment.</p>
            </div>
          )}
        </div>
      </div>
