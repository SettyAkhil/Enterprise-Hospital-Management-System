...
export default function Reports({ onNavigate }: ReportsProps) {
  ...
  const pharmData = Object.keys(pharmMap)
    .map((name) => ({
      name,
  ...
  const kpis = [
    ...
    {
      label: "Avg. Bill",
      value: "₹" + (totalRevenue / (bills.length || 1)).toFixed(0),
      change: "+0%",
    },

    {
      label: "Top Medicine",
      value: topMed,
      change: "₹" + topMedSales.toLocaleString("en-IN"),
    },
  ];
      ...
      <div className="flex gap-5">
        <div className="w-60 flex-shrink-0 space-y-2">
          {reportCategories.map((cat) => (
            <div
              key={cat.id}
              className="bg-white rounded border border-[#DDE2EC] overflow-hidden"
            >
              <p className="px-4 py-2.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider bg-[#F5F7FA] border-b border-[#F0F2F5]">
                {cat.label}
              </p>
              {cat.items.map((item) => (
                <button
                  ...
                  style={{
                    background: activeReport === item ? "#E8EDF5" : "#fff",
                    color: activeReport === item ? "#1B4FD8" : "#334155",
                  }}
                >
                  {item}
                </button>
        ...
        <div className="flex-1 space-y-5">
          <div className="bg-white rounded border border-[#DDE2EC] p-4">
            <div className="flex items-center gap-3 flex-wrap">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="px-3 py-1.5 rounded border border-[#DDE2EC] text-[12px] focus:border-[#1B4FD8] focus:outline-none"
                  />
              ...
              <button
                className="mt-4 flex items-center gap-1.5 px-4 py-1.5 rounded text-white text-[12px] font-medium"
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div className="bg-white rounded border border-[#DDE2EC] p-4">
              <p className="font-semibold text-[13px] text-[#0F1624] mb-2">
                Daily Sales Trend
              </p>
                ...
                <div className="space-y-2 flex-1">
                  {gstData.map((d) => (
                    ...
                    >
                      ...
                      <span className="text-[12px] font-semibold text-[#0F1624]">
                  ...
                  <div className="border-t border-[#F0F2F5] pt-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[13px] font-bold text-[#0F1624]">
                        ₹
                        {gstData
                ...
                {pharmData.map((p) => (
                  <div key={p.name}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <div
                          className="w-5 h-5 rounded flex items-center justify-center text-[9.5px] font-bold"
                          style={{ background: "#E8EDF5", color: "#1B4FD8" }}
                        >
                          {p.name[0]}
                        </div>
                        <span className="text-[12px] font-medium text-[#0F1624]">
                          {p.name}
                        </span>
                      </div>
                      <div className="text-right">
                        ...
                        <span className="text-[10.5px] text-[#94A3B8] ml-1">
                          ({p.bills} bills)
                        </span>
                      </div>
                    </div>
                    <div className="h-1.5 w-full bg-[#F0F2F5] rounded overflow-hidden">
                      <div
                        ...
                        style={{ width: `${p.pct}%`, background: "#1B4FD8" }}
                      />
                    </div>
                  </div>
                ))}
