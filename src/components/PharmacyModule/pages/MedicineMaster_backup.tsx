      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowModal(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-none text-white text-[13px] font-medium" style={{ background: "#4f46e5" }}>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {["Medicine Name", "Generic Name", "Strength", "Dosage Form", "Manufacturer", "Category", "MRP (₹)", "Selling Price (₹)"].map(label => (
                  <div key={label}>
                    <label className="block text-[11px] font-semibold text-[#6b7280] uppercase tracking-wide mb-1">{label}</label>
                    <input className="w-full px-3 py-2 rounded-none border border-[#e5e7eb] text-[13px] text-[#111827] focus:border-[#4f46e5] focus:outline-none transition-colors" />
                  </div>
                ))}
              </div>
              <div className="flex gap-3 pt-2">
                <button className="flex-1 py-2.5 rounded-none text-white font-semibold text-[13px]" style={{ background: "#4f46e5" }}>Add Medicine</button>
