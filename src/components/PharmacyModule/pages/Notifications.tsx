        ...
        {["All", "Unread", "Critical", "Warning", "Info"].map(f => {
          ...
          return (
            <button
              ...
              style={{ background: activeFilter === f ? "#0F1624" : "#fff", color: activeFilter === f ? "#fff" : "#334155", borderColor: activeFilter === f ? "#0F1624" : "#DDE2EC" }}
            >
              {f} <span className="text-[11px] px-1.5 py-0.5 rounded" style={{ background: activeFilter === f ? "rgba(255,255,255,0.2)" : "#F0F2F5", color: activeFilter === f ? "#fff" : "#64748B" }}>{counts[f]}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="bg-white rounded border border-[#DDE2EC] py-16 text-center">
            <div className="w-12 h-12 rounded flex items-center justify-center mx-auto mb-3" style={{ background: "#F0F2F5" }}>
          ...
          return (
            <div
              ...
              style={{ borderColor: !n.read ? cfg.border : "#DDE2EC", borderLeftWidth: !n.read ? 4 : 1 }}
            >
              <div className="flex items-start p-4 gap-4">
                <div className="w-10 h-10 rounded flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: cfg.bg }}>
                  <cfg.icon size={16} style={{ color: cfg.color }} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-[14px] font-semibold ${!n.read ? "text-[#0F1624]" : "text-[#334155]"}`}>{n.title}</p>
