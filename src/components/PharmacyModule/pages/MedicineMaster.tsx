    ...
    <div className="p-6 space-y-5">
          ...
          <tbody>
            {filtered.map(m => (
              <tr key={m.id}>
                <td>
                  <div>
                    <p className="font-semibold text-[13px] text-[#0F1624]">{m.name}</p>
                    <p className="text-[11px] text-[#94A3B8]">SKU: {m.sku}</p>
                  </div>
                </td>
                <td className="text-[13px] text-[#334155]">{m.generic}</td>
                <td className="text-[13px] font-medium text-[#334155]">{m.strength}</td>
                ...
                <td className="text-[13px] font-medium text-[#0F1624]">₹{m.mrp.toFixed(2)}</td>
                <td className="text-[13px] text-[#334155]">₹{m.price.toFixed(2)}</td>
                <td><StatusBadge status={m.status} size="sm" /></td>
                <td>
                  <div className="flex items-center gap-1">
                    <button onClick={() => setSelected(m)} className="p-1.5 rounded hover:bg-[#F0F2F5] text-[#64748B] transition-colors" title="View"><Eye size={13} /></button>
                    <button onClick={() => handleEdit(m)} className="p-1.5 rounded hover:bg-[#E8EDF5] text-[#1B4FD8] transition-colors" title="Edit"><Edit2 size={13} /></button>
                    <button onClick={() => handleDelete(m.id)} className="p-1.5 rounded hover:bg-[#FEE2E2] text-[#dc2626] transition-colors" title="Delete"><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }} onClick={() => setSelected(null)}>
          <div className="bg-white rounded shadow-2xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F2F5]">
              <div>
                <p className="font-bold text-[16px] text-[#0F1624]">{selected.name}</p>
                <p className="text-[12px] text-[#64748B]">{selected.generic} · SKU: {selected.sku}</p>
              </div>
              <button onClick={() => setSelected(null)} className="p-2 rounded hover:bg-[#F0F2F5] text-[#94A3B8] transition-colors"><X size={16} /></button>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }} onClick={() => setShowModal(false)}>
          <div className="bg-white rounded shadow-2xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F2F5]">
              <p className="font-bold text-[16px] text-[#0F1624]">Add New Medicine</p>
              <button onClick={() => setShowModal(false)} className="p-2 rounded hover:bg-[#F0F2F5] text-[#94A3B8]"><X size={16} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[11px] font-semibold text-[#64748B] uppercase tracking-wide mb-1">Medicine Name</label><input value={editingMedicine?.name || ""} onChange={e=>setEditingMedicine({...editingMedicine, name: e.target.value})} className="w-full px-3 py-2 border border-[#DDE2EC] text-[13px]" /></div>
                <div><label className="block text-[11px] font-semibold text-[#64748B] uppercase tracking-wide mb-1">Generic Name</label><input value={editingMedicine?.generic || ""} onChange={e=>setEditingMedicine({...editingMedicine, generic: e.target.value})} className="w-full px-3 py-2 border border-[#DDE2EC] text-[13px]" /></div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={handleSave} className="flex-1 py-2.5 rounded text-white font-semibold text-[13px]" style={{ background: "#1B4FD8" }}>Save Medicine</button>
