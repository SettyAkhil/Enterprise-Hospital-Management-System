      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <button onClick={() => { setSupplierId(""); setItems([]); setShowCreate(true); }} className="flex items-center gap-1.5 px-4 py-2 rounded text-white text-[13px] font-medium" style={{ background: "#1B4FD8" }}>
      ...
      <div className="bg-white rounded border border-[#DDE2EC] overflow-hidden">
        <table>
          ...
          <tbody>
            {filtered.map(po => (
              <tr key={po.id}>
                <td className="font-mono text-[12px] font-semibold" style={{ color: "#1B4FD8" }}>{po.id}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }}>
          <div className="bg-white rounded shadow-2xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F2F5]">
              <p className="font-bold text-[16px] text-[#0F1624]">Create Purchase Order</p>
              <button onClick={() => setShowCreate(false)} className="p-2 rounded hover:bg-[#F0F2F5] text-[#94A3B8]"><X size={16} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-semibold text-[#64748B] uppercase tracking-wide mb-1">Supplier *</label>
                  <select value={supplierId} onChange={e=>setSupplierId(e.target.value)} className="w-full px-3 py-2 rounded border border-[#DDE2EC] text-[13px] focus:border-[#1B4FD8] focus:outline-none transition-colors">
                    <option value="">Select supplier…</option>
                    {suppliers.map(s => <option key={s.id} value={s.id}>{s.name || s.supplierName}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-[#64748B] uppercase tracking-wide mb-1">Order Date</label>
                  <input type="date" value={poDate} onChange={e=>setPoDate(e.target.value)} className="w-full px-3 py-2 rounded border border-[#DDE2EC] text-[13px]" />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-[#64748B] uppercase tracking-wide mb-1">Expected Delivery</label>
                  <input type="date" value={expectedDate} onChange={e=>setExpectedDate(e.target.value)} className="w-full px-3 py-2 rounded border border-[#DDE2EC] text-[13px]" />
                </div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[12px] font-semibold text-[#64748B] uppercase tracking-wide">Medicines</p>
                </div>
                
                <div className="mb-4 relative">
                  <input value={medicineSearch} onChange={e=>setMedicineSearch(e.target.value)} placeholder="Search medicine to add…" className="w-full px-3 py-2 border border-[#DDE2EC] text-[13px]" />
                  {medicineSearch && (
                    <div className="absolute top-full left-0 right-0 bg-white border border-[#DDE2EC] shadow-xl max-h-48 overflow-y-auto z-10">
                      {medicines.filter(m => m.name.toLowerCase().includes(medicineSearch.toLowerCase())).map(m => (
                        <div key={m.id} onClick={() => addItem(m)} className="px-3 py-2 hover:bg-[#F5F7FA] cursor-pointer text-[13px] flex justify-between">
                          <span>{m.name}</span>
                          <span className="text-[#94A3B8]">Stock: {m.stock}</span>
                        </div>
                      ))}
                    </div>
                  )}
                    ...
                    <tbody>
                      {items.map((item, idx) => (
                        <tr key={idx}>
                          ...
                          <td><input type="number" value={item.qty} onChange={e=>{const newItems=[...items]; newItems[idx].qty=Number(e.target.value); setItems(newItems)}} className="w-20 px-2 py-1 text-[13px] border border-[#DDE2EC]" /></td>
                          <td><input type="number" value={item.price} onChange={e=>{const newItems=[...items]; newItems[idx].price=Number(e.target.value); setItems(newItems)}} className="w-24 px-2 py-1 text-[13px] border border-[#DDE2EC]" /></td>
                          <td><input type="number" value={item.gst} onChange={e=>{const newItems=[...items]; newItems[idx].gst=Number(e.target.value); setItems(newItems)}} className="w-16 px-2 py-1 text-[13px] border border-[#DDE2EC]" /></td>
                          <td className="text-[13px] font-semibold text-[#0F1624]">₹{(item.qty * item.price * (1 + item.gst/100)).toFixed(2)}</td>
                          <td><button onClick={() => setItems(items.filter((_,i)=>i!==idx))} className="p-1 rounded hover:bg-[#FEE2E2] text-[#dc2626]"><Trash2 size={12} /></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="p-5 border-t border-[#DDE2EC] flex gap-3">
              <button onClick={() => handleCreate("Submitted")} className="px-4 py-2.5 rounded text-white font-semibold text-[13px]" style={{ background: "#1B4FD8" }}>Submit for Approval</button>
              <button onClick={() => handleCreate("Draft")} className="px-4 py-2.5 rounded border border-[#DDE2EC] text-[13px] font-medium text-[#334155] hover:bg-[#F5F7FA] transition-colors">Save Draft</button>
              <button onClick={() => setShowCreate(false)} className="ml-auto px-4 py-2.5 rounded text-[13px] font-medium text-[#94A3B8] hover:text-[#334155] transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
