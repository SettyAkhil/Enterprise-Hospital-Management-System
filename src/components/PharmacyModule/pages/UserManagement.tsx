  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        actions={
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-4 py-2 rounded text-white text-[13px] font-medium" style={{ background: "#1B4FD8" }}>
      ...
      <div className="flex rounded border border-[#DDE2EC] overflow-hidden w-fit text-[13px]">
        {(["users", "permissions"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className="px-5 py-2.5 font-medium capitalize transition-colors" style={{ background: tab === t ? "#0F1624" : "#fff", color: tab === t ? "#fff" : "#64748B" }}>
      ...
      {tab === "users" ? (
        <>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {["Super Admin", "Pharmacy Manager", "Pharmacist", "Billing Operator", "Inventory Manager", "Store Keeper"].map(role => (
              <span key={role} className="text-[11px] font-medium px-2.5 py-1 rounded border border-[#DDE2EC] bg-white text-[#334155]">{role}</span>
            ))}
          </div>
                  ...
                  <tr key={u.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded flex items-center justify-center text-[12px] font-bold flex-shrink-0" style={{ background: "#E8EDF5", color: "#1B4FD8" }}>{u.avatar}</div>
                        <div>
                          <p className="font-semibold text-[13px] text-[#0F1624]">{u.name}</p>
                          <p className="text-[11px] text-[#94A3B8]">{u.email}</p>
                        </div>
                      </div>
                    </td>
                    <td>
                      <span className="text-[12px] font-medium px-2.5 py-1 rounded" style={{ background: "#F5F7FA", color: "#334155" }}>{u.role}</span>
                    </td>
        ...
        <div className="bg-white rounded border border-[#DDE2EC] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#F0F2F5] flex items-center gap-3">
            ...
            <p className="font-semibold text-[14px] text-[#0F1624]">Pharmacist Role Permissions</p>
          ...
          <table>
            ...
            <tbody>
              {permMatrix.map(row => (
                <tr key={row.module}>
                  <td className="font-semibold text-[13px] text-[#0F1624]">{row.module}</td>
                  {(["view", "create", "edit", "delete", "approve"] as const).map(perm => (
                    <td key={perm} className="text-center">
                      <span className="text-[15px]" style={{ color: row[perm] ? "#15803d" : "#DDE2EC" }}>
                        {row[perm] ? "✓" : "—"}
                      </span>
                    </td>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }}>
          <div className="bg-white rounded shadow-2xl w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F2F5]">
              <p className="font-bold text-[16px] text-[#0F1624]">Add New User</p>
