...
const quickActions = [
  { label: "New Sale", icon: ShoppingCart, color: "#0F766E", page: "dispensing" },
  { label: "Prescription Queue", icon: ClipboardList, color: "#059669", page: "prescriptions" },
...
const CHART_COLORS = { sales: "#0F766E", transactions: "#059669" };
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const kpis = [
    { title: isToday ? "Today's Total Sales" : "Selected Date Total Sales", value: `₹${selectedDateNet.toLocaleString()}`, change: "0%", up: true, sub: "Total revenue (Gross - Refunds)", icon: ShoppingCart, color: "#0F766E", bg: "#E8EDF5", page: "sales-returns" },
    { title: isToday ? "Prescriptions Today" : "Selected Date Prescriptions", value: selectedDatePrescriptions.length.toString(), change: "0%", up: true, sub: "For selected date", icon: ClipboardList, color: "#059669", bg: "#DCFCE7", page: "prescriptions" },
    { title: "Low Stock Items", value: medicines.filter(m => m.stock < m.reorderLevel).length.toString(), change: "0", up: false, sub: "Requires attention", icon: AlertTriangle, color: "#D97706", bg: "#FEF3C7", page: "expiry-low-stock" },
  ...
  const returnKpis = [
    { title: "Supplier Returns This Month", value: `₹${returnTotalMonth.toLocaleString()}`, icon: RefreshCcw, color: "#0F766E", bg: "#E8EDF5", page: "supplier-returns" },
    { title: "Pending Credit Amount", value: `₹${pendingCreditAmount.toLocaleString()}`, icon: AlertTriangle, color: "#D97706", bg: "#FEF3C7", page: "supplier-returns" },
    { title: "Expired Stock Returned", value: `${expiredStockReturned} Units`, icon: Package, color: "#dc2626", bg: "#FEE2E2", page: "supplier-returns" },
    { title: "Total Return Value", value: `₹${allTimeReturnValue.toLocaleString()}`, icon: IndianRupee, color: "#059669", bg: "#DCFCE7", page: "supplier-returns" }
  ];
  ...
  return (
    <div className="p-6 space-y-6 bg-[#F8FAFC] min-h-full font-sans text-[#0F1624]">
      <PageHeader
        ...
        title={<div className="flex items-center gap-2"><span className="text-xl">💊</span> Pharmacy Dashboard</div>}
        ...
        actions={
          <div className="flex gap-2 items-center">
            ...
            />
            <button onClick={() => onNavigate("dispensing")} style={{ background: "linear-gradient(135deg, #0F766E 0%, #059669 100%)", border: "none" }} className="px-4 py-2 rounded-lg shadow-sm text-white text-[13px] font-medium flex items-center gap-1.5 hover:shadow-md transition-shadow">
              <ShoppingCart size={14} /> New Sale
            </button>
            <button onClick={() => onNavigate("prescriptions")} className="px-4 py-2 rounded text-[13px] font-medium border border-[#E2E8F0] bg-white text-[#334155] hover:bg-[#F0FDFA] flex items-center gap-1.5 transition-colors">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map(kpi => (
          <button 
            ...
            className="bg-white rounded-xl p-5 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] border border-[#E2E8F0] border-t-[3px] hover:shadow-[0_6px_16px_-4px_rgba(0,118,110,0.15)] transition-all text-left w-full group relative overflow-hidden" style={{ borderTopColor: kpi.color }}
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="text-[12px] text-[#64748B] font-medium">{kpi.title}</p>
                <p className="text-[28px] font-bold text-[#0F1624] tracking-tight mt-1.5 leading-none">{kpi.value}</p>
              </div>
              <div className="w-10 h-10 rounded-full flex items-center justify-center border border-white/60 shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)] flex-shrink-0" style={{ background: kpi.bg }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        {returnKpis.map(kpi => (
          <button 
            key={kpi.title} 
            onClick={() => onNavigate(kpi.page)}
            className="bg-[#F8FAFC] rounded-xl p-4 shadow-sm border border-[#F1F5F9] hover:bg-white hover:border-[#E2E8F0] hover:shadow-md transition-all text-left w-full group"
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="text-[12px] text-[#64748B] font-medium">{kpi.title}</p>
                <p className="text-[22px] font-bold text-[#0F1624] tracking-tight mt-1.5 leading-none">{kpi.value}</p>
              </div>
              <div className="w-10 h-10 rounded-full flex items-center justify-center border border-white/60 shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)] flex-shrink-0" style={{ background: kpi.bg }}>
                <kpi.icon size={18} style={{ color: kpi.color }} />
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Sales Chart */}
        <div className="lg:col-span-2 bg-gradient-to-b from-[#F0FDFA] to-white rounded-xl shadow-sm border border-[#E2E8F0] border-t-[3px] border-t-[#0F766E] p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-[16px] font-bold text-[#134E4A]">Sales Overview</h2>
              <p className="text-[12px] text-[#64748B]">Revenue and transaction trends</p>
            </div>
            <div className="flex rounded-lg bg-[#F8FAFC] p-0.5 border border-[#E2E8F0] text-[12px]">
              {(["weekly", "monthly"] as const).map(v => (
                <button
                  key={v}
                  onClick={() => setChartView(v)}
                  className={`px-3 py-1.5 font-medium rounded-md transition-all ${chartView === v ? "bg-[#0F766E] text-white shadow-sm" : "bg-transparent text-[#64748B] hover:text-[#0F1624]"}`}
                >
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={chartData}>
              ...
              <Tooltip formatter={(v) => [`₹${Number(v ?? 0).toLocaleString("en-IN")}`, "Sales"]} contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #E2E8F0" }} />
              <Area type="monotone" dataKey="revenue" stroke="#0F766E" strokeWidth={2} fill="url(#salesGrad)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {quickActions.map(a => (
              <button
                ...
                className="flex flex-col items-center gap-2 p-3 rounded-xl border border-[#CCFBF1] bg-[#F0FDFA] hover:border-[#0F766E] hover:bg-[#CCFBF1] shadow-sm hover:shadow-md transition-all text-center group"
              >
                <div className="w-9 h-9 rounded flex items-center justify-center" style={{ background: `${a.color}15` }}>
                  <a.icon size={16} style={{ color: a.color }} />
                </div>
                <span className="text-[11px] font-medium text-[#334155] group-hover:text-[#0F766E] transition-colors leading-tight">{a.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-[#E2E8F0] overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#F0F2F5]">
            ...
            <button onClick={() => onNavigate("prescriptions")} className="text-[12px] font-medium flex items-center gap-1" style={{ color: "#0F766E" }}>
              View all <ArrowRight size={12} />
            </button>
          </div>
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#ECFDF5] border-y border-[#A7F3D0]"><tr>
              <th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Prescription ID</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Patient</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Doctor</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Items</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Status</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Time</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Action</th>
            </tr></thead>
            <tbody className="divide-y divide-[#D1FAE5]">
              {selectedDatePrescriptions.length === 0 ? (
                <tr><td className="py-3 px-4" colSpan={7} className="text-center py-6 text-[#64748B] text-[13px]">No prescriptions for this date.</td></tr>
              ) : selectedDatePrescriptions.slice(0, 5).map(rx => (
                <tr className="hover:bg-[#F0FDFA] transition-colors" key={rx.id}>
                  <td className="font-mono text-[12px] text-[#0F766E] font-medium py-3 px-4">{rx.id}</td>
                  <td className="py-3 px-4">
                    <p className="font-medium text-[#0F1624] text-[13px]">{rx.patient}</p>
                    <p className="text-[11px] text-[#94A3B8]">{rx.age}y · {rx.gender}</p>
                  </td>
                  <td className="text-[13px] text-[#334155] py-3 px-4">{rx.doctor}</td>
                  <td className="text-[13px] font-medium text-center py-3 px-4">{rx.items}</td>
                  <td className="py-3 px-4"><StatusBadge status={rx.status} size="sm" /></td>
                  <td className="text-[12px] text-[#94A3B8] py-3 px-4">{rx.time}</td>
                  <td className="py-3 px-4">
                    <button onClick={() => onNavigate("prescriptions")} className="text-[12px] font-medium px-2.5 py-1 rounded border border-[#E2E8F0] text-[#334155] hover:bg-[#F0F2F5] transition-colors">
                      View
                    </button>
                  </td>
                </tr>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#F0F2F5]">
            ...
            <button onClick={() => onNavigate("expiry-low-stock")} className="text-[12px] font-medium" style={{ color: "#0F766E" }}>View all</button>
          </div>
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#ECFDF5] border-y border-[#A7F3D0]"><tr>
              <th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Medicine</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Stock</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Reorder Level</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Action</th>
            </tr></thead>
            <tbody className="divide-y divide-[#D1FAE5]">
              {medicines.filter(m => m.stock < m.reorderLevel && m.stock > 0).slice(0, 5).map(m => (
                <tr className="hover:bg-[#F0FDFA] transition-colors" key={m.id}>
                  ...
                  <td className="py-3 px-4">
                    <span className="font-semibold text-[13px]" style={{ color: m.stock < 20 ? "#dc2626" : "#D97706" }}>{m.stock}</span>
                  </td>
                  <td className="text-[13px] text-[#64748B] py-3 px-4">{m.reorderLevel}</td>
                  <td className="py-3 px-4">
                    <button onClick={() => onNavigate("purchase-orders")} className="text-[11px] font-medium px-2 py-1 rounded" style={{ background: "#E8EDF5", color: "#0F766E" }}>
                      Order
                    </button>
                  </td>
                </tr>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#ECFDF5] border-y border-[#A7F3D0]"><tr>
              <th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Medicine</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Batch</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Expiry</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Days Left</th><th className="py-3 px-4 text-[11px] font-bold text-[#065F46] uppercase tracking-wider">Qty</th>
            </tr></thead>
            <tbody className="divide-y divide-[#D1FAE5]">
              {expiringMedicines.map(m => {
                const urgency = m.daysLeft <= 15 ? "#dc2626" : m.daysLeft <= 30 ? "#D97706" : "#64748B";
                return (
                  <tr className="hover:bg-[#F0FDFA] transition-colors" key={m.id}>
                    <td className="font-medium text-[13px] text-[#0F1624] py-3 px-4">{m.medicine}</td>
                    <td className="font-mono text-[12px] text-[#64748B] py-3 px-4">{m.batch}</td>
                    <td className="text-[12px] text-[#64748B] py-3 px-4">{m.expiry}</td>
                    <td className="py-3 px-4">
                      <span className="font-bold text-[13px]" style={{ color: urgency }}>{m.daysLeft}d</span>
                    </td>
                    <td className="text-[13px] text-[#334155] py-3 px-4">{m.quantity}</td>
                  </tr>
                );
              })}
            </tbody>
