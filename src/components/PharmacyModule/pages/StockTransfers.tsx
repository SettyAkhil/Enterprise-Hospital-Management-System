  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        actions={
          <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 px-4 py-2 rounded text-white text-[13px] font-medium" style={{ background: "#1B4FD8" }}>
        ...
        <div className="flex items-center gap-2">
          {workflowSteps.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded flex items-center justify-center text-[11px] font-bold" style={{ background: "#E8EDF5", color: "#1B4FD8" }}>
                  {i + 1}
                </div>
                <span className="text-[12px] font-medium text-[#334155]">{step}</span>
              </div>
        ...
        <table>
          ...
          <tbody>
            {stockTransfers.map(t => (
              <tr key={t.id}>
                <td className="font-mono text-[12px] font-semibold" style={{ color: "#1B4FD8" }}>{t.id}</td>
                <td>
                  <div className="flex items-center gap-1.5">
                    <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: "#E8EDF5" }}><Package size={11} style={{ color: "#1B4FD8" }} /></div>
                    <span className="text-[13px] text-[#0F1624]">{t.from}</span>
                  </div>
                </td>
                <td>
                  <div className="flex items-center gap-1.5">
                    <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: "#DCFCE7" }}><Package size={11} style={{ color: "#15803d" }} /></div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="date" defaultValue="2026-09-12" className="w-full px-3 py-2 rounded border border-[#DDE2EC] text-[13px] focus:border-[#1B4FD8] focus:outline-none transition-colors" />
                </div>
                <div>
                ...
                <div className="rounded border border-[#DDE2EC] overflow-hidden">
                  <table>
                    ...
                    <tbody>
                      {[1,2].map(i => (
                        <tr key={i}>
                          <td><input placeholder="Search…" className="w-full text-[13px] outline-none text-[#0F1624] placeholder:text-[#94A3B8]" /></td>
                          ...
                          <td><input type="number" defaultValue={50} className="w-20 px-2 py-1 rounded border border-[#DDE2EC] text-[13px] focus:border-[#1B4FD8] focus:outline-none" /></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
