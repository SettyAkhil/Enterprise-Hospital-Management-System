    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Transactions Logged", value: ledgerData.length + " entries", color: "#1B4FD8" },
        ].map(s => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex bg-white rounded border border-[#DDE2EC] p-1">
          {types.map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} className="px-4 py-1.5 rounded text-[12px] font-medium transition-colors" style={{
          ...
          <tbody>
            {filtered.map((r, i) => (
              <tr key={i}>
                ...
                <td className="font-mono text-[12px] font-semibold" style={{ color: "#1B4FD8" }}>{r.ref}</td>
                <td className="font-medium text-[13px] text-[#0F1624]">{r.medicine}</td>
                <td className="font-mono text-[11px] text-[#64748B]">{r.batch}</td>
                <td>
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded" style={{ background: typeColors[r.type]?.bg, color: typeColors[r.type]?.color }}>
                    {r.type}
                  </span>
                </td>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="p-8 text-center text-[#64748B] text-[13px]">No transactions found matching your criteria.</td></tr>
            )}
          </tbody>
        </table>
      </div>
