...
export default function InvoicePrintModal({ bill, returnRecord, onClose }: InvoicePrintModalProps) {
    ...
    const isReturnBill = Boolean(activeReturn || effectiveBill.isReturn || effectiveBill.billNumber?.startsWith("MOD-"));
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm print:absolute print:inset-0 print:p-0 print:bg-white">
            ...
            `}</style>
            <div id="printable-invoice" className="bg-white rounded shadow-2xl w-full max-w-4xl max-h-full overflow-y-auto print:max-w-none print:shadow-none print:w-full print:m-0 print:overflow-visible relative">
                <div className="p-6 print:p-0 border-b border-[#DDE2EC] flex justify-between items-center print:hidden bg-[#F5F7FA] sticky top-0 z-10">
                    <h2 className="text-xl font-bold text-[#0F1624] flex items-center gap-2">
                        {isReturnBill ? (
                            <>
                                ...
                                Modified Bill & Credit Note {activeReturn?.returnNumber ? `(${activeReturn.returnNumber})` : effectiveBill.billNumber}
                            </>
                        ) : (
                            <>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            <Printer size={16} /> Print {isReturnBill ? "Modified Bill" : "Receipt"}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="flex justify-between border-b-2 border-black pb-4 mb-4">
                        <div className="flex-1">
                            ...
                            <h2 className="text-lg print:text-sm font-bold uppercase mt-2">
                                {isReturnBill ? "MEDICINE RETURN CREDIT NOTE / MODIFIED INVOICE" : "Pharmacy Receipt — TAX INVOICE"}
                            </h2>
                        </div>
                        <div className="flex-1">
                            <table className="w-full text-left print:text-[9px] border-collapse">
                                <tbody>
                                    {isReturnBill ? (
                                        <>
                                            <tr>
                                                <td className="py-0.5 font-semibold">Original Bill No</td>
                                                <td>: <span className="font-bold">{activeReturn?.originalBillNumber || effectiveBill.originalBillNumber || effectiveBill.billNumber}</span></td>
                                                <td className="font-semibold w-24">Return ID</td>
                                                <td>: <span className="font-bold text-amber-700">{activeReturn?.returnNumber || "RET-2026-0001"}</span></td>
                                            </tr>
                                            <tr>
                                                <td className="py-0.5 font-semibold">Modified Bill No</td>
                                                <td>: <span className="font-bold text-indigo-700">{activeReturn?.modifiedBillNumber || effectiveBill.billNumber}</span></td>
                                                <td className="font-semibold w-24">Return Date</td>
                                                <td>: {new Date(activeReturn?.createdAt || effectiveBill.createdAt || Date.now()).toLocaleDateString()}</td>
                                            </tr>
                                            <tr>
                                                <td className="py-0.5 font-semibold">Patient Name</td>
                                                <td>: {activeReturn?.patientName || effectiveBill.patientName || "Walk-in Patient"}</td>
                                                <td className="font-semibold w-24">UMR / UHID</td>
                                                <td>: {activeReturn?.patientUhid || effectiveBill.uhid || "UMR10050"}</td>
                                            </tr>
                                            <tr>
                                                <td className="py-0.5 font-semibold">Doctor Name</td>
                                                <td>: {activeReturn?.doctorName || effectiveBill.doctorName || "Self"}</td>
                                                <td className="font-semibold w-24">Reason</td>
                                                <td>: {activeReturn?.returnReason || "Patient Return"}</td>
                                            </tr>
                                            <tr>
                                                <td className="py-0.5 font-semibold">Status</td>
                                                <td>: <span className="font-bold uppercase text-green-700">{activeReturn?.status || "Completed"}</span></td>
                                        ...
                                        <>
                                            <tr><td className="py-0.5 font-semibold">Bill No</td><td>: {effectiveBill.billNumber}</td><td className="font-semibold w-24">Bill Date</td><td>: {new Date(effectiveBill.createdAt || effectiveBill.date).toLocaleString().split(',')[0]}</td></tr>
                                            <tr><td className="py-0.5 font-semibold">UMR No</td><td>: {effectiveBill.uhid || "UMR10050"}</td><td className="font-semibold w-24">Dt of Supply</td><td>: {new Date(effectiveBill.createdAt || effectiveBill.date).toLocaleString()}</td></tr>
                                            <tr><td className="py-0.5 font-semibold">Patient Name</td><td colSpan={3}>: {effectiveBill.patientName || "Walk-in Patient"}</td></tr>
                                            <tr><td className="py-0.5 font-semibold">Doctor Name</td><td colSpan={3}>: {effectiveBill.doctorName || "Self"}</td></tr>
                    ...
                    {isReturnBill && activeReturn?.items ? (
                        <table className="w-full mb-4 print:text-[8px] table-fixed uppercase" style={{ borderCollapse: 'collapse' }}>
                            <thead className="border-y-2 border-black">
                                <tr>
                                    <th className="w-[26%] p-1 text-left font-semibold align-bottom">ITEM DESCRIPTION</th>
                                    <th className="w-[12%] p-1 text-left font-semibold align-bottom">BATCH NO</th>
                                    <th className="w-[9%] p-1 text-left font-semibold align-bottom">EXP DT</th>
                                    <th className="w-[7%] p-1 text-right font-semibold align-bottom">ORIG QTY</th>
                                    <th className="w-[7%] p-1 text-right font-semibold align-bottom text-amber-700">RET QTY</th>
                                    <th className="w-[7%] p-1 text-right font-semibold align-bottom text-indigo-700">FINAL QTY</th>
                                    <th className="w-[8%] p-1 text-right font-semibold align-bottom">RATE (₹)</th>
                                    <th className="w-[12%] p-1 text-right font-semibold align-bottom text-amber-700">REFUND (₹)</th>
                                    <th className="w-[12%] p-1 text-right font-semibold align-bottom">FINAL (₹)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {activeReturn.items.map((item: any, i: number) => {
                                    ...
                                    const finQty = item.finalQuantity !== undefined ? item.finalQuantity : (origQty - retQty);
                                    const rate = item.unitPrice || 0;
                                    const refund = item.refundAmount !== undefined ? item.refundAmount : (retQty * rate);
                                    const finalAmt = item.finalAmount !== undefined ? item.finalAmount : (finQty * rate);

                                    return (
                                        <tr key={i} className="align-top border-b border-gray-200">
                                            <td className="p-1 text-left font-medium break-words">{item.medicineName || "Medicine"}</td>
                                            <td className="p-1 text-left font-mono">{item.batchNumber || "-"}</td>
                                            <td className="p-1 text-left">{item.expiryDate || "2027-12"}</td>
                                            <td className="p-1 text-right">{origQty}</td>
                                            <td className="p-1 text-right font-bold text-amber-700">-{retQty}</td>
                                            <td className="p-1 text-right font-bold text-indigo-700">{finQty}</td>
                                            <td className="p-1 text-right">{rate.toFixed(2)}</td>
                                            <td className="p-1 text-right font-bold text-amber-700">₹{refund.toFixed(2)}</td>
                                            <td className="p-1 text-right font-semibold">₹{finalAmt.toFixed(2)}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
                            <tfoot className="border-y-2 border-black font-bold text-[9px]">
                                <tr>
                                    <td colSpan={6} className="p-1 text-left font-bold text-[10px]">
                                    ...
                                    <td className="p-1 text-right font-bold text-indigo-700">
                                        Net: ₹{(activeReturn.modifiedTotalAmount || effectiveBill.totalAmount || 0).toFixed(2)}
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                        ...
                        <table className="w-full mb-4 print:text-[8px] table-fixed uppercase" style={{ borderCollapse: 'collapse' }}>
                            ...
                            <tfoot className="border-y-2 border-black font-bold text-[9px]">
                                <tr>
                                    ...
                                    <td className="p-1 text-right"></td>
                                    <td className="p-1 text-right">{(effectiveBill.subTotal || 0).toFixed(2)}</td>
                                    <td className="p-1 text-right">{(effectiveBill.discount || 0).toFixed(2)}</td>
                                    <td className="p-1 text-right">{(effectiveBill.taxableTotal || effectiveBill.subTotal - effectiveBill.discount || 0).toFixed(2)}</td>
                                    <td className="p-1">
                                        <div className="flex justify-end px-1">
                                            <span>{(effectiveBill.cgstTotal || (effectiveBill.tax||0)/2 || 0).toFixed(2)}</span>
                                        </div>
                                    </td>
                                    <td className="p-1">
                                        <div className="flex justify-end px-1">
                                            <span>{(effectiveBill.sgstTotal || (effectiveBill.tax||0)/2 || 0).toFixed(2)}</span>
                                        </div>
                                    </td>
                                    <td className="p-1 text-right">{(effectiveBill.totalAmount || 0).toFixed(2)}</td>
                                </tr>
                            </tfoot>
                        </table>
                    )}
                            ...
                            {isReturnBill ? (
                                <div className="space-y-1">
                                    <p className="text-[12px] print:text-[9px]">
                                        Previous Bill Total: <span className="font-bold">₹{(activeReturn?.originalTotalAmount || effectiveBill.totalAmount || 0).toFixed(2)}</span>
                                    </p>
                                    <p className="text-[12px] print:text-[9px] text-amber-700 font-bold">
                                        Less: Refund / Credit Amount: -₹{(activeReturn?.refundAmount || 0).toFixed(2)}
                                    </p>
                                    <p className="text-[14px] print:text-[10px] text-indigo-900 font-bold">
                                        Modified Net Payable Total: ₹{(activeReturn?.modifiedTotalAmount ?? effectiveBill.totalAmount ?? 0).toFixed(2)}
                                    </p>
                                    <p className="mt-2 text-[11px] print:text-[8px] italic text-gray-700">
                                        Refund sum of <span className="font-bold uppercase">{numberToWords(activeReturn?.refundAmount || 0)} Rupees Only</span> approved towards medicine return.
                                    </p>
                                </div>
                            ) : (
                                <div>
                                    <p>Received sum of <span className="font-bold uppercase">{numberToWords(effectiveBill.totalAmount || 0)} Rupees Only</span> towards Above Bill</p>
                                    <div className="mt-6 border-t border-dashed border-black pt-2">
                                        <p className="font-bold mb-1">Payment Breakdown:</p>
                                        {Object.entries(effectiveBill.paymentsData?.amounts || { Cash: effectiveBill.totalAmount }).map(([method, amount]) => (
                                            <div key={method} className="flex justify-between w-48 text-[9px] uppercase">
                                                <span>{method} {(effectiveBill.paymentsData?.refs as any)?.[method] ? `(${(effectiveBill.paymentsData?.refs as any)[method]})` : ''}:</span>
