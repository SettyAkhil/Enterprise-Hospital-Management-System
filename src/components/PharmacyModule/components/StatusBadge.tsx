const configs: Record<string, { bg: string; color: string; dot: string; label?: string }> = {
  ...
  partially_received: { bg: "#FEF3C7", color: "#B45309", dot: "#D97706", label: "Partial" },
  completed:        { bg: "#DCFCE7", color: "#15803d", dot: "#16A34A" },
  pending_approval: { bg: "#FEF3C7", color: "#b45309", dot: "#f59e0b", label: "Needs Approval" },
  in_transit:       { bg: "#E8EDF5", color: "#0369a1", dot: "#0ea5e9", label: "In Transit" },
...
export default function StatusBadge({ status, size = "md" }: StatusBadgeProps) {
  const cfg = configs[status] ?? { bg: "#F5F7FA", color: "#64748B", dot: "#94A3B8" };
