        ...
        <nav className="flex items-center gap-1.5 mb-2 text-[12px] text-[#64748B]">
          {breadcrumbs.map((bc, i) => (
            <span key={i} className="flex items-center gap-1.5">
              ...
              {bc.page && onNavigate
                ? <button onClick={() => onNavigate(bc.page!)} className="hover:text-[#1B4FD8] transition-colors">{bc.label}</button>
                : <span className={i === breadcrumbs.length - 1 ? "text-[#0F1624] font-medium" : ""}>{bc.label}</span>}
            </span>
          ))}
        </nav>
        <h1 className="text-[22px] font-bold text-[#0F1624] leading-tight">{title}</h1>
