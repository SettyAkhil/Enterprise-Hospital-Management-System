...
export default function GlobalSearch({ onClose, onNavigate }: GlobalSearchProps) {
  ...
  return (
    <div className="pharmacy-module fixed inset-0 z-50 flex items-start justify-center pt-24" style={{ background: "rgba(15,23,42,0.55)", backdropFilter: "blur(4px)" }}>
      <div className="w-full max-w-2xl mx-4 bg-white rounded shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        {/* Search input */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-[#DDE2EC]">
          ...
          />
          <button onClick={onClose} className="p-1 rounded hover:bg-[#F0F2F5] text-[#94A3B8] hover:text-[#0F1624] transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto py-2">
          {filtered.map(group => (
            <div key={group.group} className="mb-1">
              <div className="flex items-center gap-2 px-5 py-2">
                ...
                <p className="text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8]">{group.group}</p>
              </div>
              {group.items.map(item => (
                ...
                >
                  <div>
                    <p className="text-[14px] font-medium text-[#0F1624]">{item.label}</p>
                    <p className="text-[12px] text-[#64748B] mt-0.5">{item.sub}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          {filtered.length === 0 && (
            <div className="px-5 py-10 text-center">
              <p className="text-[14px] text-[#94A3B8]">No results for "<span className="text-[#0F1624] font-medium">{query}</span>"</p>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-3 border-t border-[#F0F2F5] flex items-center gap-4 text-[11px] text-[#94A3B8]">
          <span><kbd className="px-1.5 py-0.5 rounded border border-[#DDE2EC] bg-[#F5F7FA] text-[10px]">↵</kbd> to select</span>
          <span><kbd className="px-1.5 py-0.5 rounded border border-[#DDE2EC] bg-[#F5F7FA] text-[10px]">↑↓</kbd> navigate</span>
          <span><kbd className="px-1.5 py-0.5 rounded border border-[#DDE2EC] bg-[#F5F7FA] text-[10px]">Esc</kbd> close</span>
        </div>
      </div>
    </div>
  );
