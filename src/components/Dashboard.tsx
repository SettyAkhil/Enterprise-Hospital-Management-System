...
const QUEUES = [
  { dept: "Emergency Department", code: "ED", waiting: 8, critical: 3, inCare: 30, available: 4, capacity: 42, color: "#E11D48", icon: Icon.Emergency },
  { dept: "Inpatient 3-North (Medical)", code: "3N", waiting: 0, critical: 2, inCare: 24, available: 2, capacity: 26, color: "#7C3AED", icon: Icon.Inpatient },
  { dept: "Inpatient 4-South (Surgical)", code: "4S", waiting: 0, critical: 1, inCare: 28, available: 4, capacity: 32, color: "#2563EB", icon: Icon.Bed },
  { dept: "Intensive Care Unit (ICU)", code: "ICU", waiting: 0, critical: 6, inCare: 12, available: 2, capacity: 14, color: "#9333EA", icon: Icon.Stethoscope },
  { dept: "Surgical Operating Rooms", code: "OR", waiting: 2, critical: 0, inCare: 3, available: 2, capacity: 5, color: "#059669", icon: Icon.Surgery },
];

const APPOINTMENTS = [
  { time: "09:00 AM", patient: "Sarah Connelly", provider: "Dr. Adams", spec: "Cardiology", room: "Rm 101", status: "Completed" },
  { time: "09:30 AM", patient: "Marcus Webb", provider: "Dr. Lee", spec: "Neurology", room: "Rm 102", status: "In Progress" },
  { time: "10:00 AM", patient: "Elena Torres", provider: "Dr. Adams", spec: "Cardiology", room: "Rm 103", status: "Checked In" },
  { time: "10:30 AM", patient: "Robert Kim", provider: "Dr. Patel", spec: "Orthopedics", room: "Rm 104", status: "Pending" },
  { time: "11:00 AM", patient: "Jennifer Walsh", provider: "Dr. Lee", spec: "Neurology", room: "Rm 102", status: "Pending" },
  { time: "11:30 AM", patient: "David Chu", provider: "Dr. Adams", spec: "General", room: "Rm 101", status: "Pending" },
];

const PENDING_LABS = [
  { patient: "John Smith", mrn: "100245", test: "BMP (Basic Metabolic)", ordered: "08:42 AM", dept: "Chemistry", status: "Critical" },
  { patient: "Mary Jones", mrn: "100246", test: "CBC w/ differential", ordered: "09:10 AM", dept: "Hematology", status: "Collected" },
  { patient: "Thomas Reed", mrn: "100301", test: "Troponin I Serial x2", ordered: "09:28 AM", dept: "Cardiac Lab", status: "Critical" },
  { patient: "Anna Weiss", mrn: "100189", test: "Urinalysis Complete", ordered: "09:55 AM", dept: "Microbiology", status: "Processing" },
];

const QUICK_ACTIONS = [
  { label: "Patient Registration", desc: "Demographics & Triage", icon: Icon.Patients, actionKey: "patients", subKey: "register", color: "#2563EB", bg: "#EFF6FF" },
  { label: "Schedule Appointment", desc: "Outpatient Consults", icon: Icon.Calendar, actionKey: "appointments", color: "#4F46E5", bg: "#EEF2FF" },
  { label: "ED Track Board", desc: "Emergency Room Grid", icon: Icon.Emergency, actionKey: "emergency", color: "#E11D48", bg: "#FFF1F2" },
  { label: "Bed Allocation Board", desc: "Inpatient Bed Census", icon: Icon.Bed, actionKey: "inpatient", color: "#7C3AED", bg: "#F5F3FF" },
  { label: "Lab Orders & Diagnostics", desc: "Chemistry, CBC, Pathology", icon: Icon.FlaskConical, actionKey: "laboratory", color: "#D97706", bg: "#FEF3C7" },
  { label: "Pharmacy Dispensing", desc: "Rx Queue & E-Prescribe", icon: Icon.Pharmacy, actionKey: "pharmacy", color: "#0891B2", bg: "#E0F2FE" },
  { label: "Surgery OR Board", desc: "Operating Suites Status", icon: Icon.Surgery, actionKey: "surgery", color: "#059669", bg: "#ECFDF5" },
  { label: "Billing & Claims", desc: "Invoices & Coverage", icon: Icon.Billing, actionKey: "billing", color: "#6366F1", bg: "#F5F3FF" },
];

const UNITS = [
  { unit: "3N Medical-Surgical", total: 32, occupied: 28, critical: 2, icon: Icon.Bed },
  { unit: "4S Special Care", total: 32, occupied: 24, critical: 1, icon: Icon.Inpatient },
  { unit: "Intensive Care Unit (ICU)", total: 14, occupied: 12, critical: 6, icon: Icon.Stethoscope },
  { unit: "Oncology 5-West", total: 24, occupied: 18, critical: 0, icon: Icon.Clinical },
      ...
      <div className="bg-white border-b border-[#DDE2EC] px-6 py-4 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Btn variant="outline" size="sm" onClick={() => setRefresh(n => n + 1)} className="shadow-sm">
      ...
      <div className="p-5 max-w-[1600px] mx-auto space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="px-5 py-3.5 border-b border-[#E2E8F0] bg-[#FAFCFF] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#64748B]">Real-time patient distribution across care units</p>
                </div>
              </div>
              <Btn variant="ghost" size="xs" onClick={() => navigate("inpatient")}>
                Full Bed Board →
              </Btn>
            </div>

            <Table headers={["Department", "In Care", "Waiting", "Critical", "Available", "Capacity", ""]}>
              {QUEUES.map((q, i) => {
                  ...
                  <TR key={i}>
                    <TD>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="font-semibold text-gray-900 text-[12.5px]">{q.dept}</div>
                          <div className="text-[10.5px] font-mono text-[#94A3B8]">{q.code} Wing</div>
                        </div>
                      </div>
                    </TD>
                    <TD>
                      <span className="font-mono text-[13px] font-bold text-gray-800">{q.inCare}</span>
                    </TD>
                    <TD>
                      {q.waiting > 0 ? (
                        ...
                        <span className="text-[#94A3B8] font-mono text-[12px]">—</span>
                      )}
                    </TD>
                    <TD>
                      {q.critical > 0 ? (
                    ...
                    <TD>
                      <div className="w-24">
                        <div className="flex items-center justify-between text-[10.5px] font-mono mb-1">
                          <span className="text-gray-600 font-medium">{occPct}%</span>
                          <span className="text-[#94A3B8]">{q.inCare}/{q.capacity}</span>
                        </div>
                        <div className="w-full h-1.5 bg-[#E2E8F0] rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${occPct}%`,
                              backgroundColor: occPct > 85 ? "#DC2626" : occPct > 70 ? "#D97706" : "#16A34A",
                            }}
                          />
                        </div>
                      </div>
                    ...
                    <TD>
                      <Btn variant="ghost" size="xs" onClick={() => navigate(q.dept.includes("Emergency") ? "emergency" : "inpatient")}>
                        View →
                      </Btn>
                    </TD>
                  </TR>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-xl shadow-sm overflow-hidden flex flex-col">
            <div className="px-4 py-3.5 border-b border-[#E2E8F0] bg-[#FAFCFF] flex items-center justify-between">
              ...
              <Btn variant="ghost" size="xs" onClick={() => navigate("appointments")}>
                View All →
              </Btn>
            </div>
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-[#64748B] truncate">
                        {a.provider} · <span className="text-[#475569]">{a.spec}</span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="px-5 py-3.5 border-b border-[#E2E8F0] bg-[#FAFCFF] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#64748B]">Active orders in processing or critical review</p>
                </div>
              </div>
              <Btn variant="ghost" size="xs" onClick={() => navigate("laboratory")}>
                Laboratory Hub →
              </Btn>
            </div>

            <Table headers={["Patient", "MRN", "Test Name", "Laboratory Dept", "Ordered Time", "Status"]}>
              {PENDING_LABS.map((l, i) => (
                <TR key={i}>
                  <TD>
                    <span className="font-bold text-gray-900 text-[12.5px]">{l.patient}</span>
                  </TD>
                  <TD>
                  ...
                  <TD>
                    <span className="font-medium text-gray-800 text-[12px]">{l.test}</span>
                  </TD>
                  <TD>
                    <span className="text-[11px] font-semibold text-[#475569]">{l.dept}</span>
                  </TD>
                  <TD>
                    <span className="font-mono text-[11.5px] text-gray-700">{l.ordered}</span>
                  </TD>
                  <TD>
                    {l.status === "Critical" ? (
                      <span className="inline-flex items-center gap-1 bg-[#FEE2E2] text-[#B91C1C] text-[11px] font-bold px-2 py-0.5 rounded-full border border-[#FECACA] animate-pulse">
                        <Icon.Alert className="w-3 h-3 text-[#DC2626]" /> Critical
                      </span>
                    ) : (
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Btn variant="outline" size="xs" onClick={() => navigate("inpatient")}>
              View All Wards →
            </Btn>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {UNITS.map((u, i) => {
              ...
              const color = pct >= 85 ? "#DC2626" : pct >= 70 ? "#D97706" : "#16A34A";
              const bgColor = pct >= 85 ? "#FEF2F2" : pct >= 70 ? "#FFFBEB" : "#F0FDF4";
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[11px] font-mono mt-1">
                    ...
                    <span className="font-semibold text-gray-600">{pct}% Occupied</span>
                  </div>
                </div>
              );
            })}
