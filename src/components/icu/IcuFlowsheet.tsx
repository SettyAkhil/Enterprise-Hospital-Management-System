...
import { FLOWSHEET_SECTIONS, HOURS, TWO_HOURLY, type FieldDef, type Section } from "./flowsheetSchema";
...
function saveRecord(rec: FlowsheetRecord) {
  try {
    localStorage.setItem(storageKey(rec.patientId, rec.date), JSON.stringify(rec));
    return true;
  } catch {
    return false;
  }
...
export async function fetchRecord(patientId: string, date: string): Promise<FlowsheetRecord | null> {
...
export async function pushRecord(rec: FlowsheetRecord, entryCount: number) {
  return apiFetch(`/api/icu/${encodeURIComponent(rec.patientId)}/flowsheet/${rec.date}`, {
    method: "PUT",
...
export function broadcastSave(patientId: string, date: string) {
  try {
    new BroadcastChannel(CHANNEL).postMessage({ patientId, date, at: Date.now() });
  } catch {}
}
export function onFlowsheetSync(handler: (m: { patientId: string; date: string }) => void) {
  try {
...
export function flowsheetSummary(patientId: string, date: string) {
  ...
  const countIn = (ids: string[]) => {
    let n = 0;
    for (const [k, v] of Object.entries(rec.fields)) if (v && ids.includes(k.split(".")[0])) n++;
    for (const id of ids) {
      ...
      for (const row of rec.tables[id] ?? []) n += Object.values(row).filter(Boolean).length;
    }
    return n;
  };
  const tabs = TABS.map((t) => ({ id: t.id, label: t.label, count: countIn(t.sections) }));
  return { tabs, total: tabs.reduce((a, b) => a + b.count, 0), updatedAt: rec.updatedAt };
}

const TABS: { id: string; label: string; sections: string[]; color: string; tint: string }[] = [
  { id: "header", label: "Chart Header", sections: ["admission", "day_plan"], color: "#1B4FD8", tint: "#EFF6FF" },
  { id: "neuro", label: "Neuro", sections: ["gcs", "rass", "pupils", "muscle_power"], color: "#7C3AED", tint: "#F5F3FF" },
  { id: "meds", label: "Medication", sections: ["drug_chart", "sos_medication", "high_risk_medication"], color: "#DB2777", tint: "#FDF2F8" },
  { id: "care", label: "Care & Lines", sections: ["pressure_ulcer", "special_care", "lines_tubes", "handover"], color: "#059669", tint: "#ECFDF5" },
  { id: "obs", label: "Hourly Observations", sections: ["observations"], color: "#DC2626", tint: "#FEF2F2" },
  { id: "io", label: "Intake / Output", sections: ["intake", "output", "balance_24h"], color: "#0891B2", tint: "#ECFEFF" },
  { id: "labs", label: "Labs & Micro", sections: ["labs", "imaging", "microbiology", "antibiotics"], color: "#D97706", tint: "#FFFBEB" },
];
...
const SYNC: Record<string, { label: string; color: string; tint: string }> = {
  ...
  offline: { label: "Offline — saved locally", color: "#B91C1C", tint: "#FEF2F2" },
};
...
function Field({
  ...
  if (def.type === "textarea") {
    return <textarea rows={3} className={cls} value={value} onChange={(e) => onChange(e.target.value)} />;
  }
  if (def.type === "check") {
    return (
      <input
...
const SectionShell = ({ title, note, children, color, tint }: { title: string; note?: string; children: React.ReactNode; color?: string; tint?: string }) => (
  <section className="bg-white border border-[#DDE2EC] shadow-sm mb-4">
    ...
    >
      <span className="w-1 h-4 flex-shrink-0" style={{ backgroundColor: color ?? "#94A3B8" }} />
      <div className="min-w-0">
        <h3 className="text-[13px] font-bold" style={{ color: color ?? "#0F172A" }}>{title}</h3>
        {note && <p className="text-[11px] text-[#64748B] mt-0.5 leading-relaxed">{note}</p>}
      </div>
    </div>
...
const thCls = "px-2 py-1.5 text-[10.5px] font-bold uppercase tracking-wide text-[#475569] bg-[#F1F5F9] border border-[#E2E8F0] whitespace-nowrap";
const tdCls = "border border-[#E2E8F0] p-0.5";
const stickyCol = "sticky left-0 z-10 bg-[#F1F5F9] border border-[#E2E8F0] px-2 py-1 text-[11px] font-semibold text-[#334155] whitespace-nowrap";

import { ensureMultiDaySeedData, getPatientStayDays, getCurrentDateTimeFormatted } from "./icuSeedData";
...
export default function IcuFlowsheet({
  ...
  const [rec, setRec] = useState<FlowsheetRecord>(() => loadRecord(patientId, today));
  const [savedAt, setSavedAt] = useState<string>("");
  const [syncState, setSyncState] = useState<"loading" | "saving" | "online" | "offline">("loading");
  ...
  useEffect(
    () =>
      onFlowsheetSync((m) => {
        if (m.patientId !== patientId || m.date !== date || dirty.current) return;
  ...
  const setField = (sid: string, key: string, v: string) => mutate((d) => { d.fields[`${sid}.${key}`] = v; });
  const setGrid = (sid: string, cell: string, v: string) => mutate((d) => { d.grids[sid] = { ...(d.grids[sid] ?? {}), [cell]: v }; });
  const setHourly = (sid: string, cell: string, v: string) => mutate((d) => { d.hourly[sid] = { ...(d.hourly[sid] ?? {}), [cell]: v }; });
  const setScale = (sid: string, cell: string, v: string) => mutate((d) => { d.scales[sid] = { ...(d.scales[sid] ?? {}), [cell]: v }; });
  ...
  const entriesFor = (sectionIds: string[]) => {
    let n = 0;
    for (const [k, v] of Object.entries(rec.fields)) if (v && sectionIds.includes(k.split(".")[0])) n++;
    for (const sid of sectionIds) {
      ...
      for (const row of rec.tables[sid] ?? []) n += Object.values(row).filter(Boolean).length;
    }
    return n;
  };
  ...
  const renderSection = (s: Section) => {
    switch (s.kind) {
      case "fields":
        return (
          <SectionShell key={s.id} title={s.title} note={s.note} color={activeTab.color} tint={activeTab.tint}>
            <div className={`grid gap-3 grid-cols-1 sm:grid-cols-2 ${s.cols === 4 ? "lg:grid-cols-4" : s.cols === 3 ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}>
              {s.fields.map((f) => (
                <div key={f.key} className={f.span === 4 ? "lg:col-span-4" : f.span === 3 ? "lg:col-span-3" : f.span === 2 ? "lg:col-span-2" : ""}>
                  <label className="block text-[11px] font-semibold text-[#475569] mb-1">
                    {f.label}
                    {f.unit && <span className="text-[#94A3B8] font-normal"> ({f.unit})</span>}
                  </label>
                  <Field def={f} value={rec.fields[`${s.id}.${f.key}`] ?? ""} onChange={(v) => setField(s.id, f.key, v)} />
                </div>
              ))}
            </div>
          </SectionShell>
              ...
              <table className="border-collapse w-full min-w-max">
                  ...
                  <tr>
                    ...
                    {s.columns.map((c) => (
                      <th key={c.key} className={thCls}>
                        {c.label}
                        {c.unit && <span className="text-[#94A3B8] normal-case"> ({c.unit})</span>}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {Array.from({ length: count }, (_, r) => (
                    <tr key={r}>
                      <td className={`${tdCls} text-center text-[11px] text-[#94A3B8] px-2`}>{r + 1}</td>
                      {s.columns.map((c) => (
                        <td key={c.key} className={tdCls} style={{ minWidth: (c.span ?? 1) * 110 }}>
                          <Field def={c} value={rows[r]?.[c.key] ?? ""} onChange={(v) => setTableCell(s.id, r, c.key, v)} compact />
                        </td>
                      ))}
                    </tr>
                  ))}
            ...
            <button
              ...
              onClick={() =>
                mutate((d) => {
                  ...
                  d.tables[s.id] = [...(d.tables[s.id] ?? Array.from({ length: count }, () => ({}))), newRow];
                })
              }
              ...
              <table className="border-collapse min-w-max">
                <thead>
                  {s.slotGroups.some((g) => g.label) && (
                    <tr>
                      <th className={`${thCls} sticky left-0 z-20`} rowSpan={2}>Care / Time</th>
                      {s.slotGroups.map((g) => (
                        <th key={g.label} className={thCls} colSpan={g.slots.length}>
                          {g.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tr>
                    {!s.slotGroups.some((g) => g.label) && <th className={`${thCls} sticky left-0 z-20`}>Care / Time</th>}
                    {s.slotGroups.map((g) =>
                      g.slots.map((sl) => (
                      ...
                      {slots.map((sl) => (
                        <td key={sl} className={tdCls} style={{ minWidth: 62 }}>
                          <input
                            ...
                            onChange={(e) => setGrid(s.id, `${row}|${sl}`, e.target.value)}
                          />
                        </td>
                      ))}
                    </tr>
      ...
      case "hourly": {
        const cols = s.groups.flatMap((g) => g.fields.map((f) => ({ group: g.label, f })));
          ...
          <SectionShell key={s.id} title={s.title} note={s.note} color={activeTab.color} tint={activeTab.tint}>
            <div className="overflow-x-auto">
              <table className="border-collapse min-w-max">
                <thead>
                  <tr>
                    <th className={`${thCls} sticky left-0 z-20`} rowSpan={2}>Time</th>
                    {s.groups.map((g) => (
                      <th key={g.label} className={thCls} colSpan={g.fields.length}>
                        {g.label}
                      </th>
                    ))}
                  </tr>
                  <tr>
                    {cols.map(({ group, f }) => (
                      <th key={`${group}-${f.key}`} className={thCls}>
                        {f.label}
                        {f.unit && <div className="text-[9px] text-[#94A3B8] normal-case">{f.unit}</div>}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {hours.map((h) => (
                    <tr key={h}>
                      ...
                      {cols.map(({ group, f }) => (
                        <td key={`${group}-${f.key}`} className={tdCls} style={{ minWidth: (f.span ?? 1) * 76 }}>
                          <Field def={f} value={rec.hourly[s.id]?.[`${h}|${f.key}`] ?? ""} onChange={(v) => setHourly(s.id, `${h}|${f.key}`, v)} compact />
                        </td>
                      ))}
                    </tr>
                  ))}
          ...
          <SectionShell key={s.id} title={s.title} note={s.note} color={activeTab.color} tint={activeTab.tint}>
            <div className="overflow-x-auto">
              <table className="border-collapse min-w-max">
                ...
                <tbody>
                  {hours.map((h) => {
                    const total = s.components.reduce((sum, c) => sum + scoreOf(h, c.key), 0);
                    const filled = s.components.every((c) => rec.scales[s.id]?.[`${h}|${c.key}`]);
                    return (
                      <tr key={h}>
                        <td className={`${stickyCol} font-mono`}>{h}</td>
                        {s.components.map((c) => (
                          <td key={c.key} className={tdCls} style={{ minWidth: 190 }}>
                            <select
                              className={`${inputBase} px-1 py-0.5 text-[11.5px]`}
                              value={rec.scales[s.id]?.[`${h}|${c.key}`] ?? ""}
                              onChange={(e) => setScale(s.id, `${h}|${c.key}`, e.target.value)}
                            >
                              <option value=""></option>
                              {c.options.map((o) => (
                        ...
                        <td className={`${tdCls} text-center`}>
                          <span
                            className={`inline-block px-2 py-0.5 font-mono text-[12px] font-bold ${
                              !filled ? "text-[#CBD5E1]" : total <= 8 ? "bg-[#FEE2E2] text-[#B91C1C]" : total <= 12 ? "bg-[#FEF3C7] text-[#B45309]" : "bg-[#DCFCE7] text-[#15803D]"
                            }`}
                          >
                            {filled ? total : "—"}
                          </span>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-[#F0F2F5]">
      ...
      <div className="bg-white border-b border-[#DDE2EC] px-6 py-3 flex flex-wrap items-center gap-4">
        <div className="min-w-0">
          <h2 className="text-[15px] font-semibold text-gray-900 truncate">ICU Daily Flowsheet — {patientName}</h2>
          <p className="text-[11.5px] text-[#64748B]">
            {bed} · MRN {patientId} · one record per patient per day
          </p>
        </div>
        <div className="flex items-center gap-2 ml-auto">
          <label className="text-[11.5px] font-semibold text-[#475569]">Date</label>
          <input
            type="date"
            value={date}
            max={today}
          ...
          >
            <span className="w-1.5 h-1.5" style={{ backgroundColor: SYNC[syncState].color }} />
            {syncState === "online" && savedAt ? `Saved ${savedAt}` : SYNC[syncState].label}
          </span>
          <Btn variant={showReports ? "primary" : "outline"} size="sm" onClick={() => setShowReports((v) => !v)}>
            {showReports ? "Back to Chart" : `Day Reports${days.length ? ` (${days.length})` : ""}`}
          </Btn>
        ...
        {getPatientStayDays(totalDays).map((sd) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] ${isSelected ? "text-blue-100" : "text-[#94A3B8]"}`}>
                ({sd.dateStr.slice(5)})
              </span>
              {sd.isToday && (
                <span className={`text-[9px] font-bold px-1 py-px ${isSelected ? "bg-white text-[#1B4FD8]" : "bg-[#EFF6FF] text-[#1B4FD8]"}`}>
                  TODAY
                </span>
              )}
            </button>
            ...
            >
              ...
              {n > 0 && (
                <span
                  ...
                  style={{ backgroundColor: active ? "#FFFFFF" : t.tint, color: t.color }}
                >
                  {n}
                </span>
              )}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <div className="bg-white border border-[#DDE2EC] shadow-sm">
            <div className="px-4 py-3 border-b border-[#EDF1F7] flex items-center justify-between">
              <div>
                <h3 className="text-[13px] font-bold text-[#1E293B]">Day-wise Chart Reports</h3>
                <p className="text-[11.5px] text-[#64748B]">
                  Every ICU chart day recorded for {patientName}. Stored in the clinical record and shown on the patient journey.
                </p>
              </div>
              <span className="text-[11.5px] font-semibold text-[#64748B]">{days.length} day{days.length === 1 ? "" : "s"}</span>
            </div>
            {days.length === 0 ? (
              <div className="px-4 py-10 text-center text-[12px] text-[#94A3B8]">
                No chart days recorded yet. Entries on the chart tabs save automatically and appear here.
              </div>
            ) : (
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  {days.map((d) => {
                    ...
                    const pct = Math.min(100, Math.round(((d.entry_count || 0) / 120) * 100));
                    return (
                      <tr key={d.chart_date} className={d.chart_date === date ? "bg-[#EFF6FF]" : "hover:bg-[#F8FAFC]"}>
                        <td className="border border-[#E2E8F0] px-2 py-1.5 font-mono font-semibold text-[#0F172A]">
                          {d.chart_date}
                          {isToday && <span className="ml-1.5 text-[10px] font-bold text-[#1B4FD8]">TODAY</span>}
                        </td>
                        <td className="border border-[#E2E8F0] px-2 py-1.5 font-mono font-bold text-[#1B4FD8]">{d.entry_count ?? 0}</td>
                        <td className="border border-[#E2E8F0] px-2 py-1.5">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-1.5 bg-[#EDF1F7] min-w-[80px]">
                              <div className="h-full" style={{ width: `${pct}%`, backgroundColor: pct > 60 ? "#16A34A" : pct > 25 ? "#D97706" : "#DC2626" }} />
                            </div>
                            <span className="font-mono text-[10.5px] text-[#64748B] w-8 text-right">{pct}%</span>
                          </div>
                        </td>
                        <td className="border border-[#E2E8F0] px-2 py-1.5 font-mono text-[11px] text-[#475569]">
                          {d.updated_at ? new Date(d.updated_at).toLocaleString() : "—"}
                        </td>
                        <td className="border border-[#E2E8F0] px-2 py-1.5 text-[11.5px] text-[#475569]">{d.updated_by || "—"}</td>
                        <td className="border border-[#E2E8F0] px-2 py-1.5">
                          <Btn
                            variant={d.chart_date === date ? "secondary" : "outline"}
                            size="xs"
                            onClick={() => {
