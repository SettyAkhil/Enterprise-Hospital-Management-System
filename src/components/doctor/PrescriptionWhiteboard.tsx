...
export default function PrescriptionWhiteboard({
  ...
  const [penType, setPenType] = useState<"ballpoint" | "fountain" | "highlighter">("fountain");
  ...
  const drawPaperBackground = useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ...
      ctx.fillText(header?.date || new Date().toLocaleDateString(), SHEET_WIDTH - 48, 76);
      ctx.textAlign = "left";
  ...
  const handlePointerUp = () => {
    ...
    if (stroke.points.length === 1) {
      stroke.points.push({ x: stroke.points[0].x + 0.5, y: stroke.points[0].y + 0.5 });
    }
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2.5 bg-white border border-[#DDE2EC] p-3 rounded-xl shadow-sm">
        ...
        <div className="flex items-center bg-[#F1F5F9] p-1 rounded-lg border border-[#E2E8F0] gap-1">
          <button
            ...
            className={`px-2.5 py-1 text-[11px] font-bold rounded-md transition-all ${
              penType === "fountain" && !erasing ? "bg-white text-[#1B4FD8] shadow-sm" : "text-[#64748B] hover:text-[#0F172A]"
            }`}
          >
            ✒️ Fountain
          </button>
          <button
            ...
            className={`px-2.5 py-1 text-[11px] font-bold rounded-md transition-all ${
              penType === "ballpoint" && !erasing ? "bg-white text-[#1B4FD8] shadow-sm" : "text-[#64748B] hover:text-[#0F172A]"
            }`}
          >
            🖊️ Fine Pen
          </button>
          <button
            ...
            className={`px-2.5 py-1 text-[11px] font-bold rounded-md transition-all ${
              penType === "highlighter" && !erasing ? "bg-white text-[#1B4FD8] shadow-sm" : "text-[#64748B] hover:text-[#0F172A]"
            }`}
          >
            🖍️ Marker
          </button>
          ...
          {PEN_COLORS.map(pen => (
            <button
              ...
              className={`w-6 h-6 rounded-full border-2 transition-all ${
                color === pen.value && !erasing ? "border-[#1B4FD8] scale-110 shadow-sm" : "border-white"
              }`}
              style={{ backgroundColor: pen.value, boxShadow: "0 0 0 1px #CBD5E1" }}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-1">
          {PEN_WIDTHS.map(pen => (
            <button key={pen.value} type="button" onClick={() => setWidth(pen.value)} className={toolButton(width === pen.value)}>
              {pen.label}
            </button>
          ))}
        </div>
        ...
        {/* Eraser & History */}
        <button type="button" onClick={() => setErasing(v => !v)} className={toolButton(erasing)}>
          🧹 Eraser
        </button>
        <button type="button" onClick={undo} disabled={!strokeCount} className={`${toolButton(false)} disabled:opacity-40 disabled:cursor-not-allowed`}>
          ↩️ Undo
        </button>
        <button type="button" onClick={redo} disabled={!canRedo} className={`${toolButton(false)} disabled:opacity-40 disabled:cursor-not-allowed`}>
          ↪️ Redo
        </button>
        <button type="button" onClick={clear} disabled={!strokeCount} className={`${toolButton(false)} disabled:opacity-40 disabled:cursor-not-allowed`}>
          🗑️ Clear Sheet
        </button>
          ...
          {PAPER_STYLES.map(style => (
            <button
              ...
              className={`px-2 py-1 text-[11px] font-semibold rounded border transition-colors ${
                paperStyle === style.id ? "bg-[#0F172A] text-white border-[#0F172A]" : "bg-white text-[#64748B] border-[#CBD5E1] hover:border-[#94A3B8]"
              }`}
            >
              {style.label}
            </button>
      ...
      >
        <canvas
          ...
          style={{ aspectRatio: `${SHEET_WIDTH} / ${SHEET_HEIGHT}`, maxWidth: "100%" }}
        />
        ...
        <div className="absolute bottom-3 right-4 bg-white/80 backdrop-blur px-3 py-1 rounded-full border border-slate-200 text-[11px] text-[#64748B] shadow-sm pointer-events-none">
          {strokeCount ? `✍️ ${strokeCount} active stroke${strokeCount === 1 ? "" : "s"}` : "🖊️ Draw or write clinical prescription here"}
        </div>
      </div>
    </div>
  );
      ...
      <div className="fixed inset-0 z-50 bg-slate-900/90 backdrop-blur-md p-6 flex flex-col justify-center overflow-y-auto">
        <div className="max-w-6xl w-full mx-auto bg-white p-4 rounded-2xl shadow-2xl space-y-4">
          <div className="flex items-center justify-between border-b pb-3">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Full-Screen Prescription Whiteboard Workspace</h2>
              <p className="text-xs text-slate-500">Fluid digital handwriting surface for high-resolution clinical notes</p>
            </div>
            <button
