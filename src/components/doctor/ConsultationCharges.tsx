...
import { MASTER_HOSPITAL_SERVICES, OrderedClinicalService } from "../DoctorWorkflow";
import type { DoctorNotification, DoctorAccount, ParsedLabTest } from "../../services/doctorPortalDb";
...
function investigationCategory(name: string): InvoiceItem["category"] {
  return name.includes("MRI") || name.includes("X-Ray") || name.includes("Ultrasound")
    ? "Radiology / Imaging"
    : "Laboratory";
}
...
export default function ConsultationCharges({
  ...
  const [sent, setSent] = useState<{ invoiceNo: string; total: number } | null>(null);
  ...
  const addService = () => {
    ...
    setOrdered(prev => {
      ...
      if (existing) {
        return prev.map(s => (s.name === def.name ? { ...s, quantity: s.quantity + qty } : s));
      }
      return [
        ...prev,
        {
  ...
  const sendToBilling = () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Could not raise the invoice. Nothing was billed.");
    }
  };
    ...
    return (
      <div className="p-6">
        <div className="max-w-2xl mx-auto bg-white border border-[#BBF7D0] rounded p-6 text-center">
          ...
          <h3 className="text-[15px] font-bold text-gray-900">Sent to billing</h3>
          <p className="text-[12.5px] text-[#475569] mt-2">
            Invoice <span className="font-mono font-semibold">{sent.invoiceNo}</span> raised for{" "}
            {patient.patientName}, total <span className="font-semibold">₹{sent.total}</span>. Reception can now take
            payment at the cashier desk.
          </p>
          <div className="mt-4">
    ...
    <div className="p-6 space-y-4">
      <div className="bg-white border border-[#DDE2EC] rounded">
        <div className="px-4 py-3 border-b border-[#DDE2EC]">
          <h3 className="text-[13px] font-bold text-gray-900">Procedures and services performed</h3>
          <p className="text-[11.5px] text-[#64748B] mt-0.5">
            Anything done in the room. The consultation fee and the investigations you ordered are added automatically.
          </p>
        </div>

        <div className="p-4 flex flex-wrap items-end gap-2">
          <label className="flex-1 min-w-[260px]">
            <span className="block text-[11px] font-semibold text-[#475569] mb-1">Service</span>
            <select
              value={serviceId}
          ...
          <label className="w-24">
            <span className="block text-[11px] font-semibold text-[#475569] mb-1">Qty</span>
            <input
              type="number"
              min={1}
              value={qty}
        ...
        {ordered.length > 0 && (
          <div className="border-t border-[#DDE2EC]">
            {ordered.map(s => (
              <div key={s.id} className="flex items-center gap-3 px-4 py-2 border-b border-[#F1F5F9] last:border-b-0">
                <div className="flex-1 min-w-0">
                  <p className="text-[12.5px] text-gray-900 truncate">{s.name}</p>
                ...
                <div className="flex items-center gap-1">
                  <button
                    type="button"
                    onClick={() => setOrdered(p => p.map(x => (x.id === s.id ? { ...x, quantity: Math.max(1, x.quantity - 1) } : x)))}
                    className="w-6 h-6 border border-[#DDE2EC] rounded text-[#475569]"
                  >
                    −
                  </button>
                  <span className="w-7 text-center text-[12.5px] font-semibold">{s.quantity}</span>
                  <button
                    type="button"
                    onClick={() => setOrdered(p => p.map(x => (x.id === s.id ? { ...x, quantity: x.quantity + 1 } : x)))}
                    className="w-6 h-6 border border-[#DDE2EC] rounded text-[#475569]"
                  >
                    +
                  </button>
                </div>
                <span className="w-20 text-right text-[12.5px] font-semibold text-gray-900">₹{s.price * s.quantity}</span>
                <button
                  type="button"
                  onClick={() => setOrdered(p => p.filter(x => x.id !== s.id))}
      ...
      <div className="bg-white border border-[#DDE2EC] rounded">
        <div className="px-4 py-3 border-b border-[#DDE2EC]">
          <h3 className="text-[13px] font-bold text-gray-900">Invoice preview</h3>
        </div>
        <div className="divide-y divide-[#F1F5F9]">
          {items.map(it => (
            <div key={it.id} className="flex items-center gap-3 px-4 py-2">
              <div className="flex-1 min-w-0">
                <p className="text-[12.5px] text-gray-900 truncate">{it.description}</p>
                <p className="text-[11px] font-mono text-[#94A3B8]">
                  {it.category} · ×{it.quantity} @ ₹{it.unitPrice}
                </p>
              </div>
              <span className="text-[12.5px] font-semibold text-gray-900">₹{it.total}</span>
            </div>
          ))}
        </div>
        <div className="px-4 py-3 border-t border-[#DDE2EC] flex items-center justify-between bg-[#F8FAFC]">
          <div className="text-[11.5px] text-[#64748B]">
            Patient payable <span className="font-semibold text-gray-900">₹{payable}</span> of ₹{total}
          </div>
      ...
      {error && (
        <div className="bg-[#FEE2E2] border border-[#FCA5A5] rounded px-4 py-2.5 text-[12px] text-[#B91C1C]">{error}</div>
      )}
