...
export default function PatientJourneyModal({
  ...
  const [activeTab, setActiveTab] = useState<'current_journey' | 'previous_consultations'>('current_journey');
  ...
  const previousEncounters = allPatientEncounters.filter(e => e.id !== encounter.id);
  const consultationRecords = DoctorPortalDatabase.getConsultationsForPatient(encounter.umr);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5 z-50 animate-in fade-in overflow-y-auto">
      <div id="printable-area" className="printable-area bg-white border-2 border-[#CBD5E1] rounded-none shadow-2xl max-w-3xl w-full my-auto flex flex-col max-h-[92vh] overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-[16px] font-bold text-white">Outpatient Clinical Journey & Consultation History</h3>
              ...
              <p className="text-[12px] text-slate-300">
                Outpatient care trajectory & past consultation data for permanent UMR: <strong className="text-white font-mono">{encounter.umr}</strong>
              </p>
            </div>
          </div>
        ...
        <div className="bg-[#F8FAFC] border-b border-[#E2E8F0] px-6 py-3 grid grid-cols-2 sm:grid-cols-4 gap-3 text-[12px] flex-shrink-0">
          <div>
            ...
            <span className="text-[11px] text-[#64748B] block">{encounter.age} yrs · {encounter.sex}</span>
          </div>
          ...
          <div>
            ...
            <span className="text-[11px] text-gray-500 block mt-0.5">{encounter.phone}</span>
          </div>

          <div>
            <span className="text-[10.5px] uppercase font-bold text-[#64748B] block">Department & Doctor</span>
            <span className="font-semibold text-gray-900 block">{encounter.dept}</span>
            <span className="text-[11px] text-gray-600 block">{encounter.assignedDoctor || "Attending Physician"} · {encounter.room || "Room 101"}</span>
          </div>

          <div>
            <span className="text-[10.5px] uppercase font-bold text-[#64748B] block">Care Status & History</span>
            ...
            <div className="relative pl-6 space-y-6 border-l-2 border-[#CBD5E1] ml-4">
                ...
                <div className="bg-white border border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono text-[11.5px] font-bold text-[#1B4FD8] bg-blue-50 px-2 py-0.5 rounded-none border border-blue-200">
                      {encounter.timestamps?.arrival || encounter.registrationTime || "10:00 AM"}
                    </span>
                  </div>
                  <p className="text-[12px] text-gray-700">
                    Patient arrived at Outpatient Reception Desk. Walk-in token initialized.
                  </p>
                  <div className="text-[11px] text-[#64748B] bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0]">
                    Token ID: <strong>{encounter.queueToken || encounter.opNumber}</strong> · Type: <strong>{encounter.isNew ? "New Outpatient" : "Returning Patient Revisit"}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="relative group">
                ...
                <div className="bg-white border border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-1.5">
                  <div className="flex justify-between items-center">
                    <div className="font-bold text-[13.5px] text-gray-900 flex items-center gap-1.5">
                      <span>📝</span> OP Registration & Permanent UMR Verification
                    </div>
                    <span className="font-mono text-[11.5px] font-bold text-[#1B4FD8] bg-blue-50 px-2 py-0.5 rounded-none border border-blue-200">
                      {encounter.timestamps?.registration || encounter.registrationTime || "10:05 AM"}
                    </span>
                  </div>
                  <p className="text-[12px] text-gray-700">
                    Demographic credentials verified against permanent master database. Lifetime <strong>{encounter.umr}</strong> linked with today's OP visit number <strong>{encounter.opNumber}</strong>.
                  </p>
                ...
                <div className="bg-white border border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-2">
                  ...
                  <div className="bg-[#F8FAFC] p-2.5 rounded-none border border-[#E2E8F0] text-[12px]">
                    ...
                    <span className="text-gray-900 font-medium">
                      "{encounter.chiefComplaint || encounter.symptoms?.join(", ") || "General outpatient evaluation requested."}"
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-[11.5px]">
                    ...
                    <span className="text-[#64748B]">Assigned to: <strong>{encounter.assignedDoctor || "Physician"}</strong></span>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white border border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-2.5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-[11.5px]">
                    <div className="bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0] text-center">
                      ...
                      <span className="font-mono font-bold text-gray-900 text-[13px]">{encounter.vitals?.bp || "120/80 mmHg"}</span>
                    </div>
                    <div className="bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0] text-center">
                      <span className="text-[#64748B] block text-[10px] uppercase font-bold">Heart Rate</span>
                      <span className="font-mono font-bold text-gray-900 text-[13px]">{encounter.vitals?.pulse || "76 bpm"}</span>
                    </div>
                    <div className="bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0] text-center">
                      <span className="text-[#64748B] block text-[10px] uppercase font-bold">Temperature</span>
                      <span className="font-mono font-bold text-gray-900 text-[13px]">{encounter.vitals?.temp || "98.6 °F"}</span>
                    </div>
                    <div className="bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0] text-center">
                      <span className="text-[#64748B] block text-[10px] uppercase font-bold">SpO2 Oxygen</span>
                      <span className="font-mono font-bold text-gray-900 text-[13px]">{encounter.vitals?.spo2 || "99%"}</span>
                    </div>
                    <div className="bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0] text-center">
                      <span className="text-[#64748B] block text-[10px] uppercase font-bold">Body Weight</span>
                      <span className="font-mono font-bold text-gray-900 text-[13px]">{encounter.vitals?.weight || "74 kg"}</span>
                    </div>
                  </div>

                  <div className="text-[11.5px] text-[#166534] bg-[#F0FDF4] p-2 rounded-none border border-emerald-200">
                    👩‍⚕️ <strong>Nurse Assessment:</strong> {encounter.vitals?.notes || "Alert, oriented, stable physiological baseline recorded."}
                  </div>
                </div>
              </div>
              ...
              <div className="relative group">
                ...
                <div className="bg-white border border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-1.5">
                  ...
                  <p className="text-[12px] text-gray-700">
                    Allocated to <strong>{encounter.assignedDoctor || "Attending Physician"}</strong> in <strong>{encounter.room || "Room 101"}</strong> with live token <strong>#{encounter.queueToken || encounter.opNumber}</strong>. Broadcasted on waiting room kiosk.
                  </p>
                </div>
              </div>
              ...
              <div className="relative group">
                ...
                <div className="bg-white border-2 border-blue-200 rounded-none p-4 shadow-xs space-y-3">
                  <div className="flex justify-between items-center border-b border-[#E2E8F0] pb-2">
                    ...
                    <span className="font-mono text-[11.5px] font-bold text-[#1B4FD8] bg-blue-50 px-2 py-0.5 rounded-none border border-blue-200">
                      {encounter.timestamps?.consultationStart || "10:32 AM"} — {encounter.timestamps?.consultationEnd || "10:45 AM"}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[12px]">
                    <div className="bg-[#F8FAFC] p-2.5 rounded-none border border-[#E2E8F0]">
                      ...
                      <span className="font-bold text-gray-900 block mt-0.5">
                        {encounter.diagnosis || "Acute Coronary Syndrome Rule-Out / Stable Angina"}
                      </span>
                      <span className="text-[11px] font-mono text-[#1B4FD8]">ICD-10: {encounter.icd10 || "I20.9"}</span>
                    </div>
                    ...
                    <div className="bg-[#F8FAFC] p-2.5 rounded-none border border-[#E2E8F0]">
                      ...
                      <span className="font-medium text-gray-800 block mt-0.5">
                        {encounter.investigations && encounter.investigations.length > 0 
                  ...
                  <div className="text-[11.5px] text-gray-700 bg-[#F8FAFC] p-2 rounded-none border border-[#E2E8F0]">
                    <strong>Doctor Advice:</strong> {encounter.advice || "Avoid strenuous exertion, follow heart-healthy diet, and follow up in 2 weeks."}
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white border border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-2">
                  ...
                  <div className="bg-[#F8FAFC] p-3 rounded-none border border-[#E2E8F0] space-y-1.5 text-[12px]">
                    <div className="flex justify-between text-gray-700">
                      ...
                      <span className="font-mono font-bold text-gray-900">₹{encounter.billing?.consultationFee || 50}.00</span>
                    </div>
                    <div className="flex justify-between text-gray-700">
                      <span>2. Diagnostic Investigations / Triage</span>
                      <span className="font-mono font-bold text-gray-900">₹{encounter.billing?.labFee || 40}.00</span>
                    </div>
                    <div className="pt-1.5 border-t border-[#E2E8F0] flex justify-between font-bold text-[13px] text-gray-900">
                      ...
                      <span className="font-mono text-emerald-700">₹{encounter.billing?.total || 90}.00 (Status: {encounter.billing?.status || 'Paid'})</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {previousEncounters.length === 0 ? (
              <div className="bg-[#F0FDF4] border-2 border-[#86EFAC] rounded-none p-6 text-center space-y-2">
                ...
                <h4 className="text-[14px] font-bold text-[#166534]">First Outpatient Visit — Initial Clinical Baseline</h4>
                <p className="text-[12px] text-[#15803D] max-w-md mx-auto">
                  This patient is registered for their first OP consultation. Today's visit will create their baseline outpatient clinical record.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="bg-[#EFF6FF] border border-[#BFDBFE] p-3 rounded-none text-[12px] text-[#1E40AF] flex items-center justify-between">
                  <span className="font-bold">📜 Chronological OP Consultation History for {encounter.patientName}</span>
                  <span className="font-mono font-bold bg-white px-2 py-0.5 border border-blue-200">{previousEncounters.length} Prior Visits</span>
                </div>

                {previousEncounters.map((pastEnc, idx) => {
                  const matchingConsult = consultationRecords.find(c => c.encounterId === pastEnc.id);

                  return (
                    <div key={pastEnc.id} className="bg-white border-2 border-[#CBD5E1] rounded-none p-4 shadow-2xs space-y-3">
                      ...
                      <div className="flex justify-between items-start border-b border-[#E2E8F0] pb-2">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-[14px] text-gray-900">{pastEnc.assignedDoctor || "Attending Physician"}</span>
                          ...
                          <span className="text-[11.5px] text-[#64748B] block mt-0.5">
                            Visit OP No: <strong className="font-mono text-gray-800">{pastEnc.opNumber}</strong> · Room: {pastEnc.room || "Room 101"}
                          </span>
                        </div>
                      ...
                      {/* Complaint & Symptoms */}
                      <div className="bg-[#F8FAFC] p-2.5 rounded-none border border-[#E2E8F0] text-[12px]">
                        <span className="font-bold text-[#64748B] block text-[10px] uppercase">Chief Complaint Narrative</span>
                        <p className="text-gray-900 font-medium mt-0.5">
                          "{pastEnc.chiefComplaint || pastEnc.symptoms?.join(", ") || "Routine follow-up evaluation."}"
                        </p>
                      </div>
                      ...
                      {pastEnc.vitals?.bp && (
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                          <div className="bg-gray-50 p-2 rounded-none border border-gray-200 text-center">
                            <span className="text-gray-500 block text-[9.5px] uppercase font-bold">BP</span>
                            <span className="font-mono font-bold text-gray-900">{pastEnc.vitals.bp}</span>
                          </div>
                          <div className="bg-gray-50 p-2 rounded-none border border-gray-200 text-center">
                            <span className="text-gray-500 block text-[9.5px] uppercase font-bold">Heart Rate</span>
                            <span className="font-mono font-bold text-gray-900">{pastEnc.vitals.pulse}</span>
                          </div>
                          <div className="bg-gray-50 p-2 rounded-none border border-gray-200 text-center">
                            <span className="text-gray-500 block text-[9.5px] uppercase font-bold">Temp</span>
                            <span className="font-mono font-bold text-gray-900">{pastEnc.vitals.temp}</span>
                          </div>
                          <div className="bg-gray-50 p-2 rounded-none border border-gray-200 text-center">
                            <span className="text-gray-500 block text-[9.5px] uppercase font-bold">SpO2</span>
                            <span className="font-mono font-bold text-gray-900">{pastEnc.vitals.spo2}</span>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[12px]">
                        <div className="bg-[#F8FAFC] p-2.5 rounded-none border border-[#E2E8F0]">
                          <span className="text-[10px] uppercase font-bold text-[#64748B] block">Diagnosis Recorded</span>
                          <span className="font-bold text-gray-900 block mt-0.5">
                            {pastEnc.diagnosis || matchingConsult?.diagnosis || "Primary Evaluation Completed"}
                          </span>
                        </div>

                        <div className="bg-[#F8FAFC] p-2.5 rounded-none border border-[#E2E8F0]">
                          <span className="text-[10px] uppercase font-bold text-[#64748B] block">Doctor Advice</span>
                          <span className="text-gray-800 font-medium block mt-0.5">
                            {pastEnc.advice || matchingConsult?.advice || "Regular follow-up as advised."}
                          </span>
                        </div>
                      </div>

                      {/* Prescribed Medications Table */}
                      {(pastEnc.prescription?.length || matchingConsult?.medications?.length) ? (
                        <div className="space-y-1 text-[11.5px]">
                          <span className="text-[10.5px] uppercase font-bold text-[#64748B] block">Medications Prescribed (Rx):</span>
                          <div className="border border-[#CBD5E1] overflow-hidden bg-white">
                            <table className="w-full text-left">
                              ...
                              <tbody className="divide-y divide-[#F1F5F9] text-[11px]">
                                {(pastEnc.prescription || matchingConsult?.medications || []).map((rx: any, rxIdx: number) => (
                                  <tr key={rxIdx}>
                                    <td className="px-3 py-1 font-semibold text-gray-900">{rx.medicine || rx.name}</td>
                                    <td className="px-3 py-1 text-gray-700">{rx.dosage}</td>
                                    <td className="px-3 py-1 text-gray-700">{rx.frequency}</td>
                                    <td className="px-3 py-1 text-gray-700">{rx.duration}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
        ...
        <div className="px-6 py-3.5 bg-slate-50 border-t border-[#E2E8F0] flex flex-col sm:flex-row sm:items-center justify-between gap-3 flex-shrink-0">
          <div className="text-[11.5px] text-[#64748B]">
            Permanent UMR: <strong>{encounter.umr}</strong> · Visit: <strong>{encounter.opNumber}</strong>
          </div>
