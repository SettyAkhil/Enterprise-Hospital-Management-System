...
import AddClinicalOrderModal from "./AddClinicalOrderModal"
...
function PatientDirectory({
  ...
  const [counts, setCounts] = useState<StreamCounts>({
    all: 0,
    op: 0,
    ip: 0,
    er: 0,
  ...
  useEffect(() => {
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        setRows(
          careFilter === "icu"
            ? patients.filter((p) => isIcuBed(p.active_bed))
            : patients,
        )
    ...
    <div className="flex-1 overflow-y-auto bg-[#F0F2F5] p-5">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-[11.5px] text-[#64748B]">
              Every patient across every department &mdash; OP, IP, ICU, and ER
              &mdash; in one place.
            </p>
          </div>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          ] as { key: CareFilter ;label: string ;count?: number }[]).map((f) => (
            <button
              key={f.key}
        ...
        <div className="bg-white border border-[#DDE2EC] rounded">
          {loading ? (
            <p className="text-[12.5px] text-[#64748B] p-4">
              Loading patients...
            </p>
          ) : rows.length === 0 ? (
            ...
            <Table
              headers={["Patient", "Age / Gender", "Location", "Doctor", ""]}
            >
              {rows.map((row) => (
...
type ClinicalOrderType = "radiology" | "nursing" | "dietary" | "referral"
type ClinicalOrderPriority = "routine" | "urgent" | "stat"
type ClinicalOrderStatus = "ordered" | "in_progress" | "completed" | "cancelled"
...
type EmrResponse = {
  ...
  clinical_orders: ClinicalOrderRow[]
  care_plan: CarePlanRow
}
...
function buildDayGroups(
  ...
  for (const o of emr.observation_notes) {
    ...
    entries.push({
      ...
      body: o.note,
      extra: o.treatment_plan
        ? `Treatment plan: ${o.treatment_plan}`
        : undefined,
    })
  }
  ...
  for (const v of emr.icu_ventilator_settings) {
    entries.push({
      ...
      body: [
        ...
        v.fio2 && `FiO2 ${v.fio2}`,
        v.peep && `PEEP ${v.peep}`,
        v.tidal_volume && `Tidal Vol ${v.tidal_volume}`,
        v.resp_rate && `Set RR ${v.resp_rate}`,
        v.pip && `PIP ${v.pip}`,
      ]
        .filter(Boolean)
        .join(" · "),
  ...
  for (const io of emr.icu_io_records) {
    entries.push({
      ...
      body: [
        io.intake_ml != null && `Intake ${io.intake_ml}mL`,
        io.output_ml != null && `Output ${io.output_ml}mL`,
      ]
        .filter(Boolean)
        .join(" · "),
...
function buildCareTeam(
  ...
  add(
    erDetail?.assigned_doctor_name,
    erDetail?.assigned_specialty ? `ER · ${erDetail.assigned_specialty}` : "ER",
  )
  for (const o of emr.observation_notes) {
    add(
      o.doctor_name,
...
export default function PatientChart({
  ...
  const [orderStatusFilter, setOrderStatusFilter] =
    useState<"all" | ClinicalOrderStatus>("all")
  const [expandedCertId, setExpandedCertId] = useState<number | null>(null)

  const buildLocalEmr = (patientId: string): {
    row: PatientRow
    emr: EmrResponse
  } | null => {
    ...
    const targetEnc = dbEncounters.find(
      (e) =>
        e.id === patientId ||
        e.umr === patientId ||
    ...
    const targetPatient = dbPatients.find(
      (p) =>
        p.umr === patientId ||
        ...
        ? [
            {
              ...
              created_at:
                targetEnc.registrationTime || new Date().toISOString(),
            },
          ]
        : [],
        ...
        ? [
            {
              ...
              created_at:
                targetEnc.timestamps?.vitalsRecorded ||
    ...
    try {
      const [emrData, erData] = await Promise.all([
        ...
        row.active_er_visit_id
          ? apiFetch<ErVisitDetail>(
              `/api/er/visits/${row.active_er_visit_id}`,
            ).catch(() => null)
  ...
  const updateOrderStatus = async (
    ...
    try {
      await apiFetch(`/api/clinical/${row.patient_id}/orders/${orderId}`, {
        method: "PATCH",
        body: JSON.stringify({ status }),
      })
      void loadPatient(row)
    } catch (error: any) {
      reportError(setNotice, error, "Failed to update this order.")
    }
  }
    ...
    ;(async () => {
      try {
        ...
        const row = (data?.patients || []).find(
          (p) =>
            p.patient_id === initialPatientId || p.name === initialPatientId,
        )
        if (row) {
          setSelectedRow(row)
          void loadPatient(row)
  ...
  const dayGroups = emr
    ? buildDayGroups(emr, erDetail)
    : new Map<string, DayEntry[]>()
  const sortedDates = Array.from(dayGroups.keys()).sort((a, b) =>
    a < b ? 1 : -1,
  )
      ...
      {selectedRow.care_stream === "OP" && (
        <div className="bg-[#EFF6FF] border-b border-[#BFDBFE] px-5 py-2.5 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 text-[12.5px] text-[#1E40AF]">
            ...
            <span>
              This patient is in Outpatient Care. Full consultation history
              &amp; care milestones are in the OP Clinical Journey.
            </span>
          </div>
          <button
            type="button"
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-0.5 text-[11.5px] text-[#64748B] flex-wrap">
                ...
                {selectedRow.age && (
                  <span>
                    {selectedRow.age}y · {selectedRow.gender || "—"}
                  </span>
                )}
          ...
          <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                      ...
                      {emr.diagnoses.length === 0 ? (
                        ...
                        <div className="space-y-1.5">
                          {emr.diagnoses.slice(0, 5).map((p) => (
                            <div
                              key={p.id}
                              className="flex items-start gap-2 py-1 border-b border-[#F1F5F9] last:border-0"
                            >
                      ...
                      {allergyList.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {allergyList.map((a) => (
                            <span
                              key={a}
                              className="bg-[#FEE2E2] text-[#B91C1C] text-[11px] font-semibold px-2 py-0.5 rounded border border-[#FECACA]"
                            >
                              {a}
                            </span>
                          ))}
                      ...
                      {emr.vitals.length === 0 ? (
                        ...
                        <div>
                          ...
                          <div className="mt-2 text-[11px] text-[#94A3B8]">
                            Last recorded:{" "}
                            {formatDateTimeIST(emr.vitals[0].created_at)}
                          </div>
                        </div>
                      )}
                    </Card>
                      ...
                      <Table headers={["Test", "Doctor", "Status", "Ordered"]}>
                        {emr.labs.slice(0, 5).map((l) => (
                          <TR key={l.id}>
                            ...
                            <TD>
                              <StatusBadge status={l.status || "due"} />
                            </TD>
                            <TD>
                ...
                <div className="space-y-4">
                  <Card title="Care Team">
                    {careTeam.length === 0 ? (
                      <p className="text-[12px] text-[#94A3B8]">
                        No doctor recorded for this patient yet.
                      </p>
                    ) : (
                      careTeam.map((c) => (
                        <div
                    ...
                    {latestAdmission ? (
                      <div className="space-y-1.5 text-[12px]">
                        {[
                          ...
                          {
                            l: "Discharge Date",
                            v: latestAdmission.discharge_date
                              ? formatDateTimeIST(
                          ...
                          <div
                            key={l}
                            className="flex justify-between py-0.5 border-b border-[#F8FAFC] last:border-0"
                          >
                      ...
                      <div className="space-y-1.5 text-[12px]">
                        {[
                          {
                            l: "Department",
                            v: selectedRow.appointment_dept || "—",
                          },
                          {
                            l: "Doctor",
                            v: selectedRow.appointment_doctor || "—",
                          },
                          {
                            l: "Status",
                            v: selectedRow.appointment_status || "—",
                          },
                        ].map(({ l, v }) => (
                          <div
                            key={l}
                    ...
                    {latestInsurance ? (
                      <div className="space-y-1.5 text-[12px]">
                        {[
                          ...
                          {
                            l: "Claim Amount",
                            v: `₹${latestInsurance.claim_amount.toLocaleString("en-IN")}`,
                          },
                          {
                            l: "Approved",
                            v: `₹${latestInsurance.approved_amount.toLocaleString("en-IN")}`,
                          },
              ...
              <div className="max-w-4xl mx-auto space-y-4">
                {sortedDates.length === 0 ? (
                  <div className="bg-white border border-[#DDE2EC] rounded p-8 text-center">
                    ...
                    <p className="text-[12px] text-[#64748B] mt-1">
                      Use "Add Evaluation" above to log today's clinical
                    ...
                    >
                      <div className="space-y-3">
                        {dayGroups.get(date)!.map((entry, i) => {
                          const color = ENTRY_TYPE_COLOR[entry.type] || {
                            bg: "#F1F5F9",
                            text: "#334155",
                          }
                          return (
                            <div
                              key={i}
                              className="pb-3 border-b border-[#F1F5F9] last:border-0 last:pb-0"
                            >
                              ...
                              {entry.body && (
                                <p className="text-[12px] text-gray-700 ml-0.5">
                                  {entry.body}
                                </p>
                              )}
                              {entry.extra && (
                                <p className="text-[11.5px] text-[#64748B] ml-0.5 mt-0.5">
                                  {entry.extra}
                                </p>
                              )}
                            </div>
                  ...
                  {emr.timeline.length === 0 ? (
                    ...
                    emr.timeline.map((e, i, arr) => (
                      <TimelineItem
                        key={i}
                        time={
                          e.timestamp ? formatDateTimeIST(e.timestamp) : "—"
                        }
                        type={e.stage}
                        title={e.label}
              ...
              <div className="space-y-4">
                <Card
                  ...
                  actions={
                    ...
                    >
                      <FiPlus aria-hidden />{" "}
                      {emr.care_plan ? "Edit Care Plan" : "New Care Plan"}
                    </Btn>
                  }
                >
                  {!emr.care_plan ||
                  (emr.care_plan.problems || []).length === 0 ? (
                    <p className="text-[12.5px] text-[#64748B]">
                      No care plan recorded yet.
                    </p>
                  ) : (
                    <Table
                      headers={["Problem", "Goal", "Status", "Target Date"]}
                    >
                      {emr.care_plan.problems.map((p) => (
                        <TR key={p.id}>
                          <TD>
                            <span className="font-medium text-gray-800">
                              {p.problem}
                ...
                <Card title="Plan Narrative">
                  {emr.care_plan?.narrative ? (
                    <p className="text-[12.5px] text-gray-700 whitespace-pre-wrap">
                      {emr.care_plan.narrative}
                    </p>
                  ) : (
                    <p className="text-[12.5px] text-[#64748B]">
                      No plan narrative recorded yet.
                    </p>
                  )}
                  {emr.care_plan?.updated_at && (
                    <p className="text-[11px] text-[#94A3B8] mt-2">
                      Last updated {formatDateTimeIST(emr.care_plan.updated_at)}
                      {emr.care_plan.updated_by
                        ? ` by ${emr.care_plan.updated_by}`
                        : ""}
                    </p>
                  )}
                </Card>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setOrderStatusFilter(
                          e.target.value as "all" | ClinicalOrderStatus,
                        )
                      }
                      className="border border-[#DDE2EC] rounded text-[11px] font-medium text-[#64748B] py-1 px-1.5 bg-white"
                    >
                    ...
                    {(emr.clinical_orders || [])
                      ...
                      .map((o) => (
                        <TR key={o.id}>
                          ...
                          <TD>
                            <span className="font-mono text-[11.5px] text-[#64748B]">
                              {formatDateTimeIST(o.ordered_at)}
                            </span>
                          </TD>
                          <TD>
                            {o.status !== "completed" &&
                                ...
                                <div className="flex gap-1.5">
                                  {o.status === "ordered" && (
                                    <button
                                      ...
                                      className="text-[11px] font-semibold text-[#1B4FD8] hover:underline"
                                    >
                                      Start
                                    </button>
                                  )}
              ...
              <div className="space-y-4">
                    ...
                    >
                      {emr.medication_schedules.map((m) => (
                        <TR key={m.id}>
                          ...
                          <TD>
                            <span className="font-mono text-[11.5px]">
                              {formatDateTimeIST(m.schedule_time)}
                            </span>
                          </TD>
                          <TD>
                            <StatusBadge
                ...
                <Card title="Pharmacy Prescriptions">
                  {emr.prescriptions.length === 0 ? (
                    <p className="text-[12.5px] text-[#64748B]">
                      No pharmacy prescriptions on record.
                    </p>
                  ) : (
                    <Table
                      headers={[
            ...
            {tab === "Allergies" && (
              <Card title="Known Allergies">
                {allergyList.length === 0 ? (
                  <p className="text-[12.5px] text-[#64748B]">
                    No known allergies on file for this patient.
                  </p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {allergyList.map((a) => (
                      <span
                        key={a}
                        className="bg-[#FEE2E2] text-[#B91C1C] text-[12px] font-semibold px-3 py-1 rounded border border-[#FECACA]"
                      >
                        ⚠ {a}
                      </span>
                    ))}
                  ...
                  <Table headers={["Recorded", "BP", "Pulse", "Temperature"]}>
                    {emr.vitals.map((v) => (
                      <TR key={v.id}>
                        <TD>
                          <span className="font-mono text-[11.5px]">
                            {formatDateTimeIST(v.created_at)}
                          </span>
                        </TD>
                        <TD>{v.bp || "—"}</TD>
                        <TD>{v.pulse || "—"}</TD>
            ...
            {tab === "Documents" && (
              <div className="space-y-4">
                <Card title="Discharge Summaries">
                  {emr.certificates.filter(
                    (c) => c.certificate_type === "discharge_summary",
                  ).length === 0 ? (
                    <p className="text-[12.5px] text-[#64748B]">
                      No discharge summary yet -- one is generated automatically
                      when this patient is discharged from a bed.
                    </p>
                  ) : (
                    <div className="space-y-2">
                      {emr.certificates
                        ...
                        .map((c) => (
                          <div
                            key={c.id}
                            className="border border-[#DDE2EC] rounded"
                          >
                            <button
                              type="button"
                ...
                <Card title="Clinical Voice Notes">
                  {emr.documents.filter(
                    (d) => d.doc_type === "clinical_audio_note",
                  ).length === 0 ? (
                    <p className="text-[12.5px] text-[#64748B]">
                      No recorded evaluations yet -- use Record in "Add
                      Evaluation" to dictate one.
                    </p>
                  ) : (
                    <div className="space-y-2">
                      {emr.documents
                        ...
                        .map((d) => (
                          <div
                            key={d.id}
                            className="border border-[#DDE2EC] rounded p-3"
                          >
                            ...
                            <audio
                              ...
                              src={`${API_BASE}/api/documents/${d.id}/file`}
                            />
                            {d.ocr_text && (
                              <p className="text-[12px] text-gray-700 mt-2 bg-[#F8FAFC] p-2 rounded border border-[#F1F5F9]">
                                <strong className="text-[11px] uppercase tracking-wide text-gray-500 block mb-0.5">
                                  Transcript
                                </strong>
                                {d.ocr_text}
                              </p>
                      ...
                      {emr.documents
                        ...
                        .map((d) => (
                          <TR key={d.id}>
                            ...
                            <TD>
                              <span className="text-[#64748B] text-[11.5px] uppercase">
                                {d.doc_type}
                              </span>
                            </TD>
                            <TD>
                            ...
                            <TD>
                              {d.has_ocr_text && (
                                <StatusBadge status="OCR Available" />
                              )}
                            </TD>
                          </TR>
                        ))}
                    ...
                    >
                      {emr.invoices.map((inv) => (
                        <TR key={inv.id}>
                          ...
                          <TD>
                            <span className="font-mono text-[12px] text-[#15803D]">
                              ₹{inv.paid_amount.toLocaleString("en-IN")}
                            </span>
                          </TD>
                          <TD>
                            <span className="font-mono text-[12px] text-[#B91C1C]">
                              ₹{inv.due_amount.toLocaleString("en-IN")}
                            </span>
                          </TD>
                          <TD>
