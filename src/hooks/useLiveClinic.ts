...
export function useLiveClinic(tickMs = 1000): LiveClinic {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => {
      ...
      if (event.key && !WATCHED_KEY_PREFIXES.some(prefix => event.key!.startsWith(prefix))) return;
      bump();
    };
...
export function formatElapsed(fromIso: string | undefined, now: number): string {
  if (!fromIso) return "--";
