 ...
 */
export function formatReceiptTimeWithSeconds(dateInput?: string | Date): string {
  try {
 ...
 */
function getServiceCode(category: string, description: string, index: number): string {
  ...
  if (desc.includes("CONSULT") || desc.includes("DR.") || desc.includes("DOCTOR")) return `DM00${30 + index}`;
  if (desc.includes("CBC") || desc.includes("BLOOD") || desc.includes("LAB")) return `LAB0${10 + index}`;
  if (desc.includes("X-RAY") || desc.includes("CT") || desc.includes("MRI")) return `RAD0${20 + index}`;
  
  if (category === "Consultation") return `DM00${25 + index}`;
  if (category === "Room / Bed Charges" || category === "Nursing") return `HSP${100 + index}`;
...
export function groupReceiptItems(items: InvoiceItem[]): ReceiptItemSection[] {
  ...
  items.forEach((item, idx) => {
    ...
    if (isDoctor) {
      subCategory = item.description.toUpperCase().includes("ENT") ? "ENT" : "GENERAL MEDICINE";
    ...
    } else if (item.description.toLowerCase().includes("oxygen") || item.description.toLowerCase().includes("emergency")) {
      subCategory = "EMERGENCY";
    } else if (item.category === "Procedure / Surgery") {
      subCategory = "PROCEDURE & OT SERVICES";
    } else if (item.category === "Room / Bed Charges" || item.category === "Nursing") {
    ...
    const serviceCode = item.cptCode && item.cptCode !== "99213" ? item.cptCode : getServiceCode(item.category, item.description, idx);
    ...
    if (isDoctor && !displayDesc.startsWith("N - ") && !displayDesc.startsWith("DR.")) {
