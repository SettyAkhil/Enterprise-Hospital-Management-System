...
export type BedSummary = { total: number; available: number; occupied: number; maintenance: number };
...
export function istLongDate(date: Date): string {
  return new Intl.DateTimeFormat("en-IN", {
    timeZone: IST, weekday: "short", day: "2-digit", month: "short", year: "numeric",
  }).format(date);
}
...
export function personName(first?: string | null, last?: string | null, fallback = "Patient"): string {
  return `${first || ""} ${last || ""}`.trim() || fallback;
}

export function erPatientLabel(v: {
  is_unknown_patient?: boolean; unknown_patient_label?: string | null;
  patient_name?: string | null; patient_last_name?: string | null;
}): string {
...
export const CLOSED_ER_STATUSES = new Set([
  "closed", "discharged", "lama", "cancelled", "left_without_being_seen",
]);
...
export function triageRank(code: string | null, categories: TriageCategory[]): number {
  if (!code) return 999;
  const found = categories.findIndex(c => c.category_code === code);
  return found === -1 ? 998 : found;
}

export function isCriticalTriage(code: string | null, categories: TriageCategory[]): boolean {
  if (!code) return false;
  const cat = categories.find(c => c.category_code === code);
  const label = (cat?.category_label || code).toLowerCase();
  return /immediate|critical|resuscitat|emergent|severe|red|p1/.test(label) || code === "B1";
}
...
export function useOpsPulse(enabled: boolean): OpsPulse {
  ...
  const load = useCallback(async () => {
    const [erRes, catRes, reqRes, pharmRes, bedsRes] = await Promise.all([
      ...
      safe(apiFetch<{ bed_requests: ErBedRequest[] }>("/api/er/bed-requests?status=pending")),
    ...
    if (bedsRes) { setBedSummary(bedsRes.summary); setBeds(bedsRes.beds); }
    setLoading(false);
  }, []);
  ...
  const critical = (active ?? []).filter(v => isCriticalTriage(v.triage_category, categories));
  ...
  for (const r of bedRequests ?? []) {
    notifications.push({
      ...
      body: `${erPatientLabel(r)} · ${r.requested_level_of_care}${r.requested_specialty ? ` · ${r.requested_specialty}` : ""}`,
      at: r.requested_at,
      module: "beds",
    });
  }
  ...
  const occupancyRate = bedSummary && bedSummary.total > 0
  ...
  const pharmacyAlerts = pharmacy === null
    ? null
  ...
  return {
    ...
    notifications,
    refresh: load,
    raw: { erVisits, triageCategories: categories, bedRequests, pharmacy, beds, bedSummary },
  };
}
