...
export function bookAppointment(booking: AppointmentBooking): BookedAppointment {
  const sex = booking.sex === "Female" ? "Female" : booking.sex === "Other" ? "Other" : "Male";
  const doctorRoom = getDoctorByName(booking.doctor)?.room;
  const room = booking.registryType === "IP" ? "Ward Pending" : doctorRoom || "Room 103";
  ...
  if (booking.encounterId) {
    ...
    db.updateEncounter(booking.encounterId, {
      ...
      timestamps: {
        ...(existing?.timestamps || { arrival: new Date().toLocaleTimeString() }),
