...
export class PharmacyDatabase {
  ...
  static processSupplierReturn(payload: Omit<AppSupplierReturn, "id" | "debitNoteNumber" | "status" | "createdAt" | "returnAmount" | "purchaseRate">, createdBy: string): AppSupplierReturn {
    ...
    this.logAudit(createdBy, "Return Created", "Inventory", newReturn.debitNoteNumber || newReturn.id, `Draft Return created for ${payload.quantity} units of batch ${batch.batchNumber}`);
    return newReturn;
  }
  ...
  static updateSupplierReturnStatus(returnId: string, newStatus: AppSupplierReturn["status"], user: string): AppSupplierReturn {
    ...
    if (newStatus === "Approved" && oldStatus !== "Approved") {
      ...
      if (batch) {
        if (batch.availableQuantity >= rtn.quantity) {
          ...
          this.logAudit(user, "Stock Deducted", "Inventory", rtn.debitNoteNumber || rtn.id, `Stock deducted for Approved Return (${rtn.quantity} units)`);
        } else {
    ...
    this.saveSupplierReturns(returns);
    this.logAudit(user, `Return ${newStatus}`, "Inventory", rtn.debitNoteNumber || rtn.id, `Return status changed from ${oldStatus} to ${newStatus}`);
