...
export type BillingDepartmentScope =
  | "All"
  | "Emergency"
  | "Inpatient"
  | "ICU"
...
const INITIAL_SEED_AUDIT_LOGS: BillingAuditRecord[] = [
  {
    ...
    newValue: "Accepted",
    financialAmount: 600,
    reason: "Inpatient stay clinical review completed and Star Health pre-authorization verified.",
  },
  {
    id: "AUDIT-2026-002",
    ...
    financialAmount: 280,
    reason: "Patient paid self-pay emergency wound treatment via UPI (Auth: UPI-AUTH-98451234).",
  },
  {
    id: "AUDIT-2026-003",
    ...
    newValue: "Appeal",
    financialAmount: 480,
    reason: "Formal medical necessity appeal submitted to Bajaj Allianz with attending microbiology charts.",
  },
];
...
const INITIAL_SEED_ADJUSTMENTS: AdjustmentRequest[] = [
  {
    ...
    reason: "Extended ICU stay copay assistance request submitted under hospital financial hardship relief policy.",
...
const INITIAL_SEED_REFUNDS: RefundRequest[] = [
  {
    ...
    reason: "Duplicate consultation co-pay collected at front desk; insurance paid in full under secondary policy.",
    status: "Pending Approval",
  },
];
...
const INITIAL_SEED_AR_ACTIVITIES: ArCollectionActivity[] = [
  {
    ...
    outcomeNotes: "Spoke with Care Health adjudications desk. Claim is in final payment batch; expected remittance in 4 business days.",
  ...
  },
  {
    ...
    outcomeNotes: "Contacted patient regarding remaining ₹120 co-pay balance. Patient promised settlement at discharge.",
...
export class BillingRbacManager {
  ...
  static onUpdate(callback: () => void): () => void {
    ...
    return () => window.removeEventListener("hospital_billing_rbac_updated", handler);
  }

  // ── Active User Management ────────────────────────────────────────────────
  static getActiveUser(): BillingUserProfile {
    return this.load<BillingUserProfile>(STORAGE_KEY_ACTIVE_USER, PRESET_BILLING_USERS.billing_manager);
  }

  static setActiveRole(role: BillingRole, department?: BillingDepartmentScope): BillingUserProfile {
  ...
  static setDepartmentScope(department: BillingDepartmentScope): BillingUserProfile {
  ...
  static hasPermission(permission: BillingPermission, user?: BillingUserProfile): boolean {
    const active = user || this.getActiveUser();
    const allowed = ROLE_PERMISSIONS[active.role] || [];
    return allowed.includes(permission);
  }

  static checkPermission(permission: BillingPermission, actionLabel = "this action"): void {
    if (!this.hasPermission(permission)) {
      const active = this.getActiveUser();
      throw new Error(`Access Denied: Your role '${active.roleLabel}' does not have permission to ${actionLabel} (Required: ${permission}).`);
    }
  }

  static isDepartmentAuthorized(department: string, user?: BillingUserProfile): boolean {
    const active = user || this.getActiveUser();
    if (active.role !== "dept_billing" || active.department === "All") return true;
    return active.department.toLowerCase() === department.toLowerCase();
  }

  // ── Audit Trail Methods ───────────────────────────────────────────────────
  static getAuditLogs(filter?: { search?: string; action?: string; patientName?: string }): BillingAuditRecord[] {
    let logs = this.load<BillingAuditRecord[]>(STORAGE_KEY_AUDIT_LOGS, INITIAL_SEED_AUDIT_LOGS);
    if (filter) {
  ...
  static logEvent(data: Omit<BillingAuditRecord, "id" | "timestamp" | "userId" | "userName" | "userRole">): BillingAuditRecord {
    const active = this.getActiveUser();
  ...
  static getAdjustmentRequests(status?: "All" | "Pending Approval" | "Approved" | "Rejected"): AdjustmentRequest[] {
    let reqs = this.load<AdjustmentRequest[]>(STORAGE_KEY_ADJUSTMENT_REQS, INITIAL_SEED_ADJUSTMENTS);
  ...
  static requestAdjustment(params: {
    ...
    const active = this.getActiveUser();
    if (!this.hasPermission("adjustments.create_small") && !this.hasPermission("adjustments.create_large")) {
      throw new Error("Access Denied: You do not have permission to request adjustments or write-offs.");
    }
  ...
  static reviewAdjustment(
    ...
    const active = this.getActiveUser();
    this.checkPermission("adjustments.approve", "approve or reject financial write-offs and adjustments");
    ...
    if (current.requestedBy === active.name && !active.canApproveSelf) {
      throw new Error("Regulatory Compliance Violation: You cannot approve your own financial adjustment request. Another authorized manager must review it.");
    }
    ...
    const updated: AdjustmentRequest = {
      ...
      reviewNotes: reviewNotes || (action === "approve" ? "Approved in accordance with hospital revenue cycle guidelines." : "Rejected during audit review."),
    };
    ...
    this.logEvent({
      ...
      reason: `Adjustment ₹${current.amount.toLocaleString("en-IN")} ${action === "approve" ? "APPROVED" : "REJECTED"}: ${updated.reviewNotes}`,
  ...
  static getRefundRequests(status?: "All" | "Pending Approval" | "Approved" | "Rejected"): RefundRequest[] {
    let reqs = this.load<RefundRequest[]>(STORAGE_KEY_REFUND_REQS, INITIAL_SEED_REFUNDS);
    if (status && status !== "All") {
      reqs = reqs.filter((r) => r.status === status);
    }
    return reqs.sort((a, b) => new Date(b.requestedAt).getTime() - new Date(a.requestedAt).getTime());
  ...
  static requestRefund(params: {
    ...
    const active = this.getActiveUser();
    this.checkPermission("payments.refund_request", "initiate a patient payment refund request");
  ...
  static reviewRefund(
    ...
    const active = this.getActiveUser();
    this.checkPermission("payments.refund_approve", "approve or reject patient refunds");
    ...
    // Anti-Self-Approval Enforcement
    if (current.requestedBy === active.name && !active.canApproveSelf) {
      throw new Error("Regulatory Compliance Violation: You cannot approve your own refund request. Another manager or finance controller must review it.");
    }
    ...
    const updated: RefundRequest = {
      ...
      reviewedBy: active.name,
      reviewedAt: new Date().toISOString(),
      reviewNotes: reviewNotes || (action === "approve" ? "Refund authorized and processed to origin payment method." : "Refund rejected during manager audit."),
    };
    ...
    this.logEvent({
      ...
      invoiceNo: current.invoiceId,
      financialAmount: current.amount,
      reason: `Patient refund of ₹${current.amount.toLocaleString("en-IN")} ${action === "approve" ? "APPROVED" : "REJECTED"}: ${updated.reviewNotes}`,
      approvedBy: active.name,
    });
  ...
  static getArActivities(invoiceNo?: string): ArCollectionActivity[] {
    let acts = this.load<ArCollectionActivity[]>(STORAGE_KEY_AR_ACTIVITIES, INITIAL_SEED_AR_ACTIVITIES);
    if (invoiceNo) {
      acts = acts.filter((a) => a.invoiceNo === invoiceNo || a.claimId === invoiceNo);
    }
  ...
  static logArActivity(params: {
    ...
    const active = this.getActiveUser();
    this.checkPermission("ar.add_activity", "log A/R collection follow-ups and payment promises");
