    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 bg-white border border-[#e2e8f0] rounded-lg px-3 py-2 flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, generic, SKU…" className="text-[13px] outline-none text-[#0f172a] placeholder:text-[#94a3b8] w-full" />
        ...
        <div className="ml-auto flex items-center gap-2 text-[13px] text-[#64748b]">
          <span className="font-medium text-[#0f172a]">{filtered.length}</span> results
        </div>
      </div>
          ...
          <tbody>
            {filtered.map(m => (
              <tr key={m.id}>
                ...
                <td className="text-[13px] font-medium text-[#0f172a]">₹{m.mrp.toFixed(2)}</td>
                <td className="text-[13px] text-[#374151]">₹{m.price.toFixed(2)}</td>
                <td><StatusBadge status={m.status} size="sm" /></td>
                <td>
                  <div className="flex items-center gap-1">
                    <button onClick={() => setSelected(m)} className="p-1.5 rounded hover:bg-[#f1f5f9] text-[#64748b] transition-colors" title="View"><Eye size={13} /></button>
                    <button className="p-1.5 rounded hover:bg-[#eff6ff] text-[#2563eb] transition-colors" title="Edit"><Edit2 size={13} /></button>
                    <button className="p-1.5 rounded hover:bg-[#fef2f2] text-[#dc2626] transition-colors" title="Delete"><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }} onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#f1f5f9]">
              ...
              <button onClick={() => setSelected(null)} className="p-2 rounded-lg hover:bg-[#f1f5f9] text-[#94a3b8] transition-colors"><X size={16} /></button>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }} onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#f1f5f9]">
              <p className="font-bold text-[16px] text-[#0f172a]">Add New Medicine</p>
              <button onClick={() => setShowModal(false)} className="p-2 rounded-lg hover:bg-[#f1f5f9] text-[#94a3b8]"><X size={16} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {["Medicine Name", "Generic Name", "Strength", "Dosage Form", "Manufacturer", "Category", "MRP (₹)", "Selling Price (₹)"].map(label => (
                  <div key={label}>
                    <label className="block text-[11px] font-semibold text-[#64748b] uppercase tracking-wide mb-1">{label}</label>
              ...
              <div className="flex gap-3 pt-2">
                <button className="flex-1 py-2.5 rounded-xl text-white font-semibold text-[13px]" style={{ background: "#2563eb" }}>Add Medicine</button>
      ...
      <div className="grid grid-cols-2 gap-2">
        {rows.map(([label, value]) => (
          <div key={label} className="p-3 rounded-lg bg-[#f8fafc]">
            ...
            <p className="text-[13px] font-semibold text-[#0f172a] mt-0.5">{value}</p>
          </div>
        ))}
      </div>
    </div>
