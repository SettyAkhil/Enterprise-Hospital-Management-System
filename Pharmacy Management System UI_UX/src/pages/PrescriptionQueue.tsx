...
import { Search, Filter, X, User, Stethoscope, FileText, CheckCircle, Play, XCircle, MessageSquare, ChevronRight } from "lucide-react";
...
export default function PrescriptionQueue({ onNavigate }: PrescriptionQueueProps) {
  ...
  const rxMeds = [
    { name: "Metformin 500mg", dosage: "1 tablet", frequency: "Twice daily", duration: "30 days", instructions: "After meals" },
    { name: "Amlodipine 5mg", dosage: "1 tablet", frequency: "Once daily", duration: "30 days", instructions: "Morning, with water" },
    { name: "Atorvastatin 10mg", dosage: "1 tablet", frequency: "Once daily", duration: "30 days", instructions: "At bedtime" },
    { name: "Pantoprazole 40mg", dosage: "1 tablet", frequency: "Once daily", duration: "14 days", instructions: "Before breakfast" },
  ];
          ...
          <div className="flex items-center gap-3 mt-4 mb-0 flex-wrap">
            <div className="flex rounded-lg border border-[#e2e8f0] overflow-hidden bg-white text-[13px]">
              {filters.map(f => (
                <button
                  ...
                  style={{ background: activeFilter === f ? "#0f172a" : "#fff", color: activeFilter === f ? "#fff" : "#64748b" }}
                >
                  {f}
                  {f === "Pending" && <span className="ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold" style={{ background: activeFilter === f ? "rgba(255,255,255,0.2)" : "#fef2f2", color: activeFilter === f ? "#fff" : "#dc2626" }}>4</span>}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2 ml-auto">
              <div className="flex items-center gap-2 bg-white border border-[#e2e8f0] rounded-lg px-3 py-2">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search patient, doctor, ID…" className="text-[13px] outline-none text-[#0f172a] placeholder:text-[#94a3b8] w-44" />
              </div>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={rx.id} onClick={() => setSelected(rx)} className="cursor-pointer">
                    <td className="font-mono text-[12px] font-semibold" style={{ color: "#2563eb" }}>{rx.id}</td>
                    <td>
      ...
      {selected && (
        <div className="w-96 bg-white border-l border-[#e2e8f0] flex flex-col flex-shrink-0 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#f1f5f9]">
            <div>
              <p className="font-semibold text-[14px] text-[#0f172a]">{selected.id}</p>
              <p className="text-[12px] text-[#64748b]">{selected.date} · {selected.time}</p>
            </div>
            <button onClick={() => setSelected(null)} className="p-1.5 rounded-lg hover:bg-[#f1f5f9] text-[#94a3b8] transition-colors">
              <X size={16} />
            </button>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {rxMeds.slice(0, selected.items).map((m, i) => (
                  <div key={i} className="p-3 rounded-lg border border-[#f1f5f9] bg-[#f8fafc]">
                    ...
                    <p className="text-[11px] text-[#94a3b8] mt-0.5 italic">{m.instructions}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="p-4 border-t border-[#e2e8f0] space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <button className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-white text-[12px] font-medium" style={{ background: "#0f766e" }}>
                <CheckCircle size={13} /> Verify
              </button>
              <button className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-white text-[12px] font-medium" style={{ background: "#2563eb" }}>
...
function Row({ label, value, children }: { label: string; value?: string; children?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      {children ?? <span className="text-[13px] text-[#0f172a] font-medium text-right">{value}</span>}
    </div>
  );
}

function ClipboardIcon() {
  return <div className="w-12 h-12 rounded-xl bg-[#f1f5f9] flex items-center justify-center mx-auto"><FileText size={22} className="text-[#94a3b8]" /></div>;
}
