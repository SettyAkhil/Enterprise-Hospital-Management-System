  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
        <p className="text-[12px] font-semibold text-[#64748b] uppercase tracking-wide mb-4">Transfer Workflow</p>
        <div className="flex items-center gap-2">
          {workflowSteps.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold" style={{ background: "#eff6ff", color: "#2563eb" }}>
                  {i + 1}
                </div>
        ...
        <table>
          ...
          <tbody>
            {stockTransfers.map(t => (
              <tr key={t.id}>
                <td className="font-mono text-[12px] font-semibold" style={{ color: "#2563eb" }}>{t.id}</td>
                <td>
                  <div className="flex items-center gap-1.5">
                    <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: "#eff6ff" }}><Package size={11} style={{ color: "#2563eb" }} /></div>
                ...
                <td className="text-[13px] text-[#64748b]">{t.approvedBy ?? <span className="text-[#94a3b8]">Pending</span>}</td>
                ...
                <td>
                  <div className="flex items-center gap-1">
                    ...
                    {t.status === "in_transit" && (
                      <button className="flex items-center gap-1 text-[11px] font-semibold px-2 py-1 rounded" style={{ background: "#eff6ff", color: "#2563eb" }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl mx-4 max-h-[85vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#f1f5f9]">
              <p className="font-bold text-[16px] text-[#0f172a]">Create Stock Transfer</p>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-[#64748b] uppercase tracking-wide mb-1">Transfer Date</label>
                  <input type="date" defaultValue="2026-09-12" className="w-full px-3 py-2 rounded-lg border border-[#e2e8f0] text-[13px] focus:border-[#2563eb] focus:outline-none transition-colors" />
                </div>
                <div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button className="flex items-center gap-1 text-[12px] font-medium" style={{ color: "#2563eb" }}><Plus size={12} /> Add</button>
                </div>
                <div className="rounded-xl border border-[#e2e8f0] overflow-hidden">
                  <table>
                    ...
                    <tbody>
                      {[1,2].map(i => (
                        <tr key={i}>
                          <td><input placeholder="Search…" className="w-full text-[13px] outline-none text-[#0f172a] placeholder:text-[#94a3b8]" /></td>
                          <td><input placeholder="Batch" className="w-20 text-[12px] outline-none font-mono text-[#64748b]" /></td>
                          <td className="text-[13px] text-[#64748b]">—</td>
                          <td><input type="number" defaultValue={50} className="w-20 px-2 py-1 rounded border border-[#e2e8f0] text-[13px] focus:border-[#2563eb] focus:outline-none" /></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="p-5 border-t border-[#e2e8f0] flex gap-3">
              <button className="flex-1 py-2.5 rounded-xl text-white font-semibold text-[13px]" style={{ background: "#2563eb" }}>Submit Request</button>
              <button onClick={() => setShowCreate(false)} className="flex-1 py-2.5 rounded-xl border border-[#e2e8f0] text-[13px] font-medium text-[#374151] hover:bg-[#f8fafc] transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
