  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        actions={
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-white text-[13px] font-medium" style={{ background: "#2563eb" }}>
      ...
      <div className="flex rounded-xl border border-[#e2e8f0] overflow-hidden w-fit text-[13px]">
        {(["users", "permissions"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className="px-5 py-2.5 font-medium capitalize transition-colors" style={{ background: tab === t ? "#0f172a" : "#fff", color: tab === t ? "#fff" : "#64748b" }}>
      ...
      {tab === "users" ? (
        <>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {["Super Admin", "Pharmacy Manager", "Pharmacist", "Billing Operator", "Inventory Manager", "Store Keeper"].map(role => (
              <span key={role} className="text-[11px] font-medium px-2.5 py-1 rounded-full border border-[#e2e8f0] bg-white text-[#374151]">{role}</span>
            ))}
          </div>
                  ...
                  <tr key={u.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold flex-shrink-0" style={{ background: "#eff6ff", color: "#2563eb" }}>{u.avatar}</div>
                        <div>
                          <p className="font-semibold text-[13px] text-[#0f172a]">{u.name}</p>
                          <p className="text-[11px] text-[#94a3b8]">{u.email}</p>
                        </div>
                      </div>
                    </td>
                    <td>
                      <span className="text-[12px] font-medium px-2.5 py-1 rounded-full" style={{ background: "#f8fafc", color: "#374151" }}>{u.role}</span>
                    </td>
                    <td className="text-[13px] text-[#374151]">{u.branch}</td>
                    <td className="text-[12px] text-[#64748b]">{u.lastLogin}</td>
                    ...
                    <td>
                      <div className="flex items-center gap-1">
                        ...
                        <button className="p-1.5 rounded hover:bg-[#f1f5f9] text-[#64748b] transition-colors" title="Toggle">
                          {u.status === "active" ? <ToggleRight size={14} style={{ color: "#0f766e" }} /> : <ToggleLeft size={14} />}
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#f1f5f9] flex items-center gap-3">
            ...
            <p className="font-semibold text-[14px] text-[#0f172a]">Pharmacist Role Permissions</p>
          ...
          <table>
            ...
            <tbody>
              {permMatrix.map(row => (
                <tr key={row.module}>
                  <td className="font-semibold text-[13px] text-[#0f172a]">{row.module}</td>
                  {(["view", "create", "edit", "delete", "approve"] as const).map(perm => (
                    <td key={perm} className="text-center">
                      <span className="text-[15px]" style={{ color: row[perm] ? "#15803d" : "#e2e8f0" }}>
                        {row[perm] ? "✓" : "—"}
                      </span>
                    </td>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#f1f5f9]">
              <p className="font-bold text-[16px] text-[#0f172a]">Add New User</p>
              <button onClick={() => setShowAdd(false)} className="p-2 rounded-lg hover:bg-[#f1f5f9] text-[#94a3b8]"><X size={16} /></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button className="flex-1 py-2.5 rounded-xl text-white font-semibold text-[13px]" style={{ background: "#2563eb" }}>Create User</button>
                <button onClick={() => setShowAdd(false)} className="flex-1 py-2.5 rounded-xl border border-[#e2e8f0] text-[13px] font-medium text-[#374151] hover:bg-[#f8fafc] transition-colors">Cancel</button>
              </div>
            </div>
          </div>
        </div>
