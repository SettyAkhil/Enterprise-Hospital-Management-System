...
const typeConfig = {
  ...
  info:     { icon: Info, color: "#2563eb", bg: "#eff6ff", border: "#93c5fd", dot: "#60a5fa" },
};
  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        actions={
          <button onClick={markAll} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-[#e2e8f0] bg-white text-[13px] text-[#374151] hover:bg-[#f8fafc] transition-colors">
        ...
        {["All", "Unread", "Critical", "Warning", "Info"].map(f => {
          ...
          return (
            <button
              ...
              style={{ background: activeFilter === f ? "#0f172a" : "#fff", color: activeFilter === f ? "#fff" : "#374151", borderColor: activeFilter === f ? "#0f172a" : "#e2e8f0" }}
            >
              {f} <span className="text-[11px] px-1.5 py-0.5 rounded-full" style={{ background: activeFilter === f ? "rgba(255,255,255,0.2)" : "#f1f5f9", color: activeFilter === f ? "#fff" : "#64748b" }}>{counts[f]}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-xl border border-[#e2e8f0] py-16 text-center">
            ...
            <p className="text-[12px] text-[#94a3b8] mt-1">You're all caught up!</p>
          </div>
        ) : filtered.map(n => {
              ...
              <div className="flex items-start p-4 gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-[14px] font-semibold ${!n.read ? "text-[#0f172a]" : "text-[#374151]"}`}>{n.title}</p>
                  ...
                  <p className="text-[13px] text-[#64748b] mt-1">{n.message}</p>
                  <div className="flex items-center gap-3 mt-2">
                    {!n.read && (
                      <button onClick={() => markOne(n.id)} className="text-[12px] font-medium" style={{ color: "#2563eb" }}>Mark as read</button>
                    )}
                    <button className="text-[12px] font-medium text-[#64748b] hover:text-[#0f172a] transition-colors">View details</button>
                    <button onClick={() => remove(n.id)} className="ml-auto p-1 rounded hover:bg-[#fef2f2] text-[#94a3b8] hover:text-[#dc2626] transition-colors">
                      <X size={12} />
                    </button>
                  </div>
                </div>
