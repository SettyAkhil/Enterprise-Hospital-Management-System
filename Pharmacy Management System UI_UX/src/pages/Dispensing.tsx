...
import { Search, X, Plus, Minus, Printer, CreditCard, Smartphone, Wallet, ShieldCheck, ReceiptText, Trash2, ChevronDown } from "lucide-react";
...
export default function Dispensing({ onNavigate }: DispensingProps) {
  ...
  const updateQty = (id: number, delta: number) => {
    setCart(prev => prev.map(i => i.id === id ? { ...i, qty: Math.max(1, i.qty + delta), total: Math.max(1, i.qty + delta) * i.mrp * (1 - i.discount/100) * (1 + i.tax/100) } : i));
  };
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-4">
                ...
                {filteredMeds.map(m => (
                  ...
                  >
                    ...
                    <div className="text-right ml-4">
                      <p className="text-[13px] font-bold text-[#0f172a]">₹{m.mrp}</p>
                      <p className="text-[11px]" style={{ color: m.stock > 50 ? "#15803d" : m.stock > 0 ? "#d97706" : "#dc2626" }}>
                        {m.stock > 0 ? `Stock: ${m.stock}` : "Out of stock"}
                      </p>
                    </div>
                    <div className="ml-3 w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#eff6ff" }}>
          ...
          <div className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
            <table>
              <thead><tr>
                <th>Medicine</th><th>Batch</th><th>Expiry</th><th>Qty</th><th>MRP (₹)</th><th>Disc%</th><th>Tax%</th><th>Total (₹)</th><th></th>
              </tr></thead>
              <tbody>
                {cart.length === 0 ? (
                  <tr><td colSpan={9} className="py-12 text-center text-[#94a3b8]">
                    ...
                    <p className="text-[12px] mt-1">Search and add medicines above</p>
                  </td></tr>
                ) : cart.map(item => (
                  <tr key={item.id}>
                    <td className="font-medium text-[13px] text-[#0f172a]">{item.medicine}</td>
                    <td className="font-mono text-[11px] text-[#64748b]">{item.batch}</td>
                    <td className="text-[12px] text-[#64748b]">{item.expiry}</td>
                    <td>
                      <div className="flex items-center gap-1">
                        <button onClick={() => updateQty(item.id, -1)} className="w-6 h-6 rounded border border-[#e2e8f0] flex items-center justify-center hover:bg-[#f1f5f9] transition-colors">
                          <Minus size={11} />
                        </button>
                    ...
                    <td className="text-[13px] text-[#64748b]">{item.tax}%</td>
                    <td className="font-semibold text-[13px] text-[#0f172a]">₹{item.total.toFixed(2)}</td>
                    <td>
                      <button onClick={() => removeItem(item.id)} className="p-1.5 rounded hover:bg-[#fef2f2] text-[#dc2626] transition-colors">
                        <Trash2 size={13} />
                      </button>
                    </td>
                  </tr>
      ...
      <div className="w-72 xl:w-80 bg-white border-l border-[#e2e8f0] flex flex-col flex-shrink-0">
        <div className="p-5 border-b border-[#f1f5f9]">
          <h2 className="text-[15px] font-semibold text-[#0f172a]">Billing Summary</h2>
          <p className="text-[12px] text-[#64748b]">{cart.length} items · {cart.reduce((s,i)=>s+i.qty,0)} units</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="space-y-2.5">
            {[
              ...
              { label: `Discount (${discount}%)`, value: -discountAmt, color: "#15803d" },
              ...
              <div key={r.label} className="flex items-center justify-between text-[13px]">
                ...
                <span className="font-medium" style={{ color: r.color ?? "#0f172a" }}>
            ...
            <div className="flex items-center justify-between text-[15px] font-bold border-t border-[#e2e8f0] pt-2.5">
              ...
              <span style={{ color: "#2563eb" }}>₹{(Math.round(grand)).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <input type="range" min={0} max={30} value={discount} onChange={e => setDiscount(+e.target.value)} className="w-full accent-[#2563eb]" />
          </div>

          {/* Payment methods */}
          <div>
            <p className="text-[12px] font-semibold text-[#64748b] uppercase tracking-wide mb-2">Payment Method</p>
        ...
        <div className="p-5 border-t border-[#e2e8f0] space-y-2">
          <button className="w-full py-3 rounded-xl text-white font-semibold text-[14px] flex items-center justify-center gap-2 transition-all hover:opacity-90 active:scale-[0.98]" style={{ background: "#2563eb" }}>
