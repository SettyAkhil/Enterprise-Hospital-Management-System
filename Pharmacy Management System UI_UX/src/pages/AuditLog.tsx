    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 bg-white border border-[#e2e8f0] rounded-lg px-3 py-2">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search user, record, action…" className="text-[13px] outline-none text-[#0f172a] placeholder:text-[#94a3b8] w-48" />
        </div>
        <div className="flex gap-1">
          {modules.map(m => (
            <button key={m} onClick={() => setModuleFilter(m)} className="px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors border border-[#e2e8f0]" style={{ background: moduleFilter === m ? "#0f172a" : "#fff", color: moduleFilter === m ? "#fff" : "#64748b" }}>
              {m}
            </button>
          ))}
        </div>
                ...
                <tr key={i} onClick={() => setExpanded(expanded === i ? null : i)} className="cursor-pointer">
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold" style={{ background: "#eff6ff", color: "#2563eb" }}>{log.user[0]}</div>
                      <span className="text-[13px] font-medium text-[#0f172a]">{log.user}</span>
                    </div>
                  </td>
                  <td>
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "#f8fafc", color: "#374151" }}>{log.module}</span>
                  </td>
                  <td>
                    <span className="text-[12px] font-bold" style={{ color: actionColors[log.action] ?? "#374151" }}>{log.action}</span>
                  </td>
                  <td className="font-mono text-[12px] font-semibold" style={{ color: "#2563eb" }}>{log.record}</td>
                  <td className="font-mono text-[11px] text-[#94a3b8]">{log.ip}</td>
                  <td>
                    <button className="flex items-center gap-1 text-[12px] font-medium" style={{ color: "#2563eb" }}>
                  ...
                  <tr key={`exp-${i}`}>
                    <td colSpan={7} style={{ background: "#f8fafc", padding: 0 }}>
                      <div className="px-14 py-3 flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-[12px] font-medium text-[#0f172a] mb-1">Change Details</p>
                          <p className="text-[12px] text-[#64748b]">{log.details}</p>
                          <p className="text-[11px] text-[#94a3b8] mt-1">Recorded at {log.timestamp} · IP: {log.ip}</p>
                        </div>
                      </div>
                    </td>
                  </tr>
