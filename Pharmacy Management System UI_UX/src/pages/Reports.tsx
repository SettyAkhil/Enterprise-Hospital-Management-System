...
const reportCategories = [
  ...
  { id: "purchasing", label: "Purchasing Reports", items: ["Supplier Purchases", "Purchase Summary", "Purchase Returns"] },
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#e2e8f0] bg-white text-[13px] text-[#374151] hover:bg-[#f8fafc] transition-colors"><Printer size={13} /> Print</button>
            <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#e2e8f0] bg-white text-[13px] text-[#374151] hover:bg-[#f8fafc] transition-colors"><FileText size={13} /> PDF</button>
            <button className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-white text-[13px] font-medium" style={{ background: "#0f766e" }}><Download size={14} /> Export Excel</button>
          </div>
        }
        ...
        <div className="w-60 flex-shrink-0 space-y-2">
          {reportCategories.map(cat => (
            <div key={cat.id} className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
              <p className="px-4 py-2.5 text-[11px] font-bold text-[#94a3b8] uppercase tracking-wider bg-[#f8fafc] border-b border-[#f1f5f9]">{cat.label}</p>
              {cat.items.map(item => (
                <button
                  key={item}
                  ...
                  style={{ background: activeReport === item ? "#eff6ff" : "#fff", color: activeReport === item ? "#2563eb" : "#374151" }}
                >
                  {item}
                </button>
              ))}
        ...
        <div className="flex-1 space-y-5">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="date" defaultValue="2026-09-01" className="px-3 py-1.5 rounded-lg border border-[#e2e8f0] text-[12px] focus:border-[#2563eb] focus:outline-none" />
                  <span className="text-[#94a3b8]">—</span>
                  <input type="date" defaultValue="2026-09-12" className="px-3 py-1.5 rounded-lg border border-[#e2e8f0] text-[12px] focus:border-[#2563eb] focus:outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-[#94a3b8] uppercase tracking-wide mb-1">Branch</label>
                <select className="px-3 py-1.5 rounded-lg border border-[#e2e8f0] text-[12px] focus:border-[#2563eb] focus:outline-none">
                  <option>All Branches</option><option>Main Branch</option><option>Branch 2</option>
                </select>
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-[#94a3b8] uppercase tracking-wide mb-1">Category</label>
                <select className="px-3 py-1.5 rounded-lg border border-[#e2e8f0] text-[12px] focus:border-[#2563eb] focus:outline-none">
                  <option>All Categories</option><option>Antibiotic</option><option>Analgesic</option>
                </select>
              </div>
              <button className="mt-4 flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-white text-[12px] font-medium" style={{ background: "#2563eb" }}>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { label: "Top Medicine", value: "Azithromycin", change: "₹18,200 sold" },
            ].map(k => (
              <div key={k.label} className="bg-white rounded-xl p-4 border border-[#e2e8f0]">
                <p className="text-[11px] text-[#64748b] font-medium">{k.label}</p>
                <p className="text-[17px] font-bold text-[#0f172a] mt-1">{k.value}</p>
          ...
          <div className="grid grid-cols-2 gap-5">
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
              <p className="font-semibold text-[14px] text-[#0f172a] mb-4">Daily Sales Trend</p>
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
              <p className="font-semibold text-[14px] text-[#0f172a] mb-4">Medicine-wise Sales</p>
              <ResponsiveContainer width="100%" height={200}>
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
              <p className="font-semibold text-[14px] text-[#0f172a] mb-4">GST Collection Breakup</p>
                ...
                <div className="space-y-3 flex-1">
                  {gstData.map(d => (
                    <div key={d.name} className="flex items-center justify-between">
                      ...
                      <span className="text-[13px] font-semibold text-[#0f172a]">₹{d.value.toLocaleString("en-IN")}</span>
                    </div>
                  ))}
                  <div className="border-t border-[#f1f5f9] pt-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[12px] font-semibold text-[#64748b]">Total GST</span>
                      <span className="text-[14px] font-bold text-[#0f172a]">₹{gstData.reduce((s,d)=>s+d.value,0).toLocaleString("en-IN")}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
              <p className="font-semibold text-[14px] text-[#0f172a] mb-4">Pharmacist-wise Performance</p>
              <div className="space-y-3">
                {[
                  { name: "Kavitha Ramesh", bills: 52, value: "₹38,420", pct: 82 },
                  { name: "Deepak Subramaniam", bills: 43, value: "₹31,850", pct: 68 },
                  { name: "Priya Nair", bills: 33, value: "₹13,980", pct: 52 },
                ].map(p => (
                  <div key={p.name}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold" style={{ background: "#eff6ff", color: "#2563eb" }}>{p.name[0]}</div>
                        <span className="text-[13px] font-medium text-[#0f172a]">{p.name}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[13px] font-semibold text-[#0f172a]">{p.value}</span>
