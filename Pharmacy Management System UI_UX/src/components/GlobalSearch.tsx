    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-24" style={{ background: "rgba(15,23,42,0.6)", backdropFilter: "blur(4px)" }}>
      <div className="w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-[#e2e8f0]">
          ...
          />
          <button onClick={onClose} className="p-1 rounded hover:bg-[#f1f5f9] text-[#94a3b8] hover:text-[#0f172a] transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto py-2">
          ...
          {filtered.length === 0 && (
            <div className="px-5 py-10 text-center">
              <p className="text-[14px] text-[#94a3b8]">No results for "<span className="text-[#0f172a] font-medium">{query}</span>"</p>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-3 border-t border-[#f1f5f9] flex items-center gap-4 text-[11px] text-[#94a3b8]">
          <span><kbd className="px-1.5 py-0.5 rounded border border-[#e2e8f0] bg-[#f8fafc] text-[10px]">↵</kbd> to select</span>
          <span><kbd className="px-1.5 py-0.5 rounded border border-[#e2e8f0] bg-[#f8fafc] text-[10px]">↑↓</kbd> navigate</span>
          <span><kbd className="px-1.5 py-0.5 rounded border border-[#e2e8f0] bg-[#f8fafc] text-[10px]">Esc</kbd> close</span>
        </div>
      </div>
    </div>
  );
