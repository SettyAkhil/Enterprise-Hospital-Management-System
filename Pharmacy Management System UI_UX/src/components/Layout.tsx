...
export default function Layout({ activePage, onNavigate, children, onGlobalSearch }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9]">
      ...
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 py-4 border-b border-[#1e293b]" style={{ minHeight: 64 }}>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#0f766e" }}>
            <Activity size={16} className="text-white" />
          </div>
          {!collapsed && (
            <div className="overflow-hidden">
              <p className="text-white font-semibold text-sm leading-tight truncate">MedCare Pharmacy</p>
              <p className="text-[11px]" style={{ color: "#64748b" }}>Main Branch</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-1">
          {navItems.map((group) => (
            <div key={group.group} className="mb-2">
              {!collapsed && (
                <p className="px-2 mb-1 text-[10px] font-semibold uppercase tracking-widest" style={{ color: "#475569" }}>
                  {group.group}
                </p>
              )}
                ...
                return (
                  ...
                  >
                    ...
                    {!collapsed && "badge" in item && item.badge && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: active ? "rgba(255,255,255,0.2)" : "#dc2626", color: "#fff" }}>
                        {item.badge}
                      </span>
                    )}
                    {collapsed && "badge" in item && item.badge && (
        ...
        <div className="border-t border-[#1e293b] p-2">
          {!collapsed && (
            <div className="flex items-center gap-2 px-2 py-2 rounded-md mb-1" style={{ color: "#94a3b8" }}>
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: "#0f766e", color: "#fff" }}>
                KR
              </div>
              <div className="overflow-hidden flex-1">
                <p className="text-[12px] font-medium text-white truncate">Kavitha Ramesh</p>
                <p className="text-[11px]" style={{ color: "#64748b" }}>Pharmacist</p>
              </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Header */}
        <header className="flex items-center gap-4 px-6 bg-white border-b border-[#e2e8f0]" style={{ height: 64, flexShrink: 0 }}>
          {/* Global search */}
          <button
          ...
          >
            ...
            <span className="ml-auto text-[11px] font-medium px-1.5 py-0.5 rounded border border-[#e2e8f0] bg-[#f8fafc] text-[#64748b]">Ctrl+K</span>
          </button>

          <div className="flex items-center gap-1 ml-auto">
            {/* Date / time */}
            <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[#64748b]" style={{ fontSize: 12 }}>
              <Calendar size={13} />
              <span>{dateStr} · {timeStr}</span>
            </div>

            {/* Branch badge */}
            <span className="hidden lg:inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium" style={{ background: "#f0fdf4", color: "#15803d" }}>
              Main Branch
            </span>
              ...
              {showNotifications && (
                <div className="absolute right-0 top-full mt-2 w-96 bg-white rounded-xl shadow-2xl border border-[#e2e8f0] z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[#f1f5f9]">
                    <p className="font-semibold text-[14px] text-[#0f172a]">Notifications</p>
                    <button onClick={() => onNavigate("notifications")} className="text-xs font-medium" style={{ color: "#2563eb" }}>View all</button>
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.slice(0, 5).map(n => (
                      <div key={n.id} className={`px-4 py-3 border-b border-[#f8fafc] hover:bg-[#f8fafc] transition-colors ${!n.read ? "bg-blue-50/30" : ""}`}>
                        <div className="flex gap-3">
                          <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${n.type === "critical" ? "bg-red-500" : n.type === "warning" ? "bg-amber-500" : "bg-blue-400"}`} />
                          <div>
            ...
            <button className="flex items-center gap-2 ml-1 p-1.5 rounded-lg hover:bg-[#f1f5f9] transition-colors">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: "#0f766e", color: "#fff" }}>KR</div>
            </button>
          </div>
        </header>
