...
async function warmUp(port, urlPath, attempts = 20) {
  for (let i = 0; i < attempts; i++) {
    try {
      const res = await fetch(`http://127.0.0.1:${port}${urlPath}`, { signal: AbortSignal.timeout(30_000) });
...
if (await portInUse(OCR_PORT)) {
  ...
  console.warn(`[keppler-ocr] ${OCR_DIR} not found -- skipping (Keppler OCR tab will 502)`);
} else {
